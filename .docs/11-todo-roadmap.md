...
- [ ] **P0.1 — Hợp đồng "mồ côi" trong `Abstractions`** — 4 interface khai báo nhưng không class nào implement/consume (grep toàn solution chỉ ra đúng 4 file khai báo). Xử lý 3 nhánh:
  - [ ] **Sửa comment** [IDatagramTransport.cs:5](../src/TqkLibrary.Vpn.Abstractions/Transport/Interfaces/IDatagramTransport.cs#L5): bỏ tên type ma `EspIkeDemuxTransport` (không tồn tại ở đâu), trỏ về hiện thực thật [`Ipsec/Nat`](../src/TqkLibrary.Vpn.Ipsec/Nat) (`NatTraversal`/`NatTraversalChannel`). **Giữ** interface làm nền `Transport.Dtls` (**F.3**).
  - [ ] **Làm thật `IByteStreamTransport`**: refactor [SstpTransport.cs](../src/TqkLibrary.Vpn.Drivers.Sstp/SstpTransport.cs) tách 2 lớp — (a) TLS byte-stream `TcpClient`+`SslStream` **implement** [`IByteStreamTransport`](../src/TqkLibrary.Vpn.Abstractions/Transport/Interfaces/IByteStreamTransport.cs) (Connect/Read/Write thuần), (b) framing SSTP 4-byte header bên trên; inject (a) qua **factory** vào [SstpConnection.cs](../src/TqkLibrary.Vpn.Drivers.Sstp/SstpConnection.cs) → tạo **seam test offline** cho supervisor/keepalive/reconnect SSTP (mở khóa **P1.6**) + nền tách `Transport.Tls` (**F.1**). **Sửa kèm**: honor `CancellationToken` trong `ConnectAsync` ([SstpTransport.cs:33-47](../src/TqkLibrary.Vpn.Drivers.Sstp/SstpTransport.cs#L33-L47)).
  - [ ] **Xóa `ISecuritySession` + `IPacketEncapsulator`** khỏi `Abstractions` ([ISecuritySession.cs](../src/TqkLibrary.Vpn.Abstractions/Security/Interfaces/ISecuritySession.cs), [IPacketEncapsulator.cs](../src/TqkLibrary.Vpn.Abstractions/Encapsulation/Interfaces/IPacketEncapsulator.cs)) — shape thực tế đã phân kỳ: [`EspSession`](../src/TqkLibrary.Vpn.Ipsec/Esp/EspSession.cs) dùng `TryUnprotect` bool + rekey/sequence-exposure (vs `Unprotect` trả −1, không slot rekey); framing thật là HDLC event-driven / `L2tpCodec` / SSTP header (vs `TryDecode` trên `ReadOnlySequence`). **Tái thiết kế thành mục F.2** — khi driver V đầu tiên cần (OpenVPN data-channel / WireGuard-Noise / CSTP framing), thiết kế từ ≥2 consumer thật thay vì thiết kế chay. Cập nhật bảng design-diff trong `10` + README `Abstractions`.
...
> P0.1/P0.4/P0.6/P0.7/P0.8 cũng thuộc 2 driver này nhưng nằm ở §P0 (làm trước).
...
- [ ] **P1.6 — Test offline supervisor/keepalive/reconnect SSTP**: chèn transport giả lập qua seam factory `IByteStreamTransport` (có sau **P0.1**); hiện supervisor SSTP chỉ được phủ qua live integration test. → [SstpConnection.cs](../src/TqkLibrary.Vpn.Drivers.Sstp/SstpConnection.cs).
...
- [ ] **F.1 — `Transport.Tcp`/`Transport.Tls`** (implement [`IByteStreamTransport`](../src/TqkLibrary.Vpn.Abstractions/Transport/Interfaces/IByteStreamTransport.cs)): tách TLS-over-TCP từ `SstpTransport` (sau P0.1) ra project dùng chung + cert-callback (P0.6). Consumer: SSTP, SoftEther (V.4), OpenConnect CSTP (V.5), OpenVPN-TCP (V.2).
...
|---|---|---|---|---|---|---|
| SSTP (live) | TCP+TLS → F.1 | TLS | SSTP 4-byte | ✅ | — | P0.1/P0.6 |
...
0. **P0** — dọn kiến trúc & nợ review (P0.1 → P0.2 → … → P0.11, như §P0).
