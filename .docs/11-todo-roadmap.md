...
1. **P1** — SSTP + L2TP/IPsec đạt 100%: P1.1 IPV6CP → P1.2 outer IPv6 → P1.3 rekey Phase 1 in-place → P1.5 timeout SSTP read (P1.4 retransmit-backoff đã xong, as-built ở [`10`](10-codebase-architecture-and-flow.md) §9 + README L2tp/L2tpIpsec) → P1.6 test SSTP offline → P1.7 multi-session L2TP (best-effort). **Q.1 lab Docker dựng sớm** trong giai đoạn này (điều kiện test P1.1/P1.2/P1.7 + P0.8).
