...
{
    /// <summary>
    /// What an <see cref="IL2tpv3EthTransportFactory"/> hands back: the UDP datagram pipe the data plane rides
    ...
    /// in-process loopback delivers itself by invoking the handler directly, so its pump is <c>null</c>. The pipe is disposed
    /// when the attempt ends. Mirrors <c>GeneveTransportHandle</c>.
    /// </summary>
    public sealed class L2tpv3EthTransportHandle
    {
        /// <summary>Creates a handle around <paramref name="datagram"/>.</summary>
        public L2tpv3EthTransportHandle(IDatagramTransport datagram,
        ...
        /// <summary>The UDP datagram pipe (one datagram = one L2TPv3 data message; there is no framing).</summary>
