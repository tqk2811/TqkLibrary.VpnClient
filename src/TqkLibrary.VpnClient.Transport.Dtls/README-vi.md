...
| `OpenConnectDtls12PskClient` | (internal) `PskTlsClient`: **DTLS 1.2 PSK full handshake** (cipher OpenConnect `PSK-NEGOTIATE`) — `GetSupportedVersions`=DTLS 1.2, suite `TLS_PSK_WITH_AES_256/128_GCM`, PSK identity/key từ `DtlsPskParameters`; ép `ClientHello.session_id` = `X-DTLS-App-ID` bằng `TlsUtilities.ImportSession` (session throwaway: dummy master + suite PSK + DTLSv12 + no-EMS, `GetSessionToResume` + `AllowLegacyResumption`=true) — gateway trả full handshake (không resume thật), né hẳn bug legacy resumption | [OpenConnectDtls12PskClient.cs:21](OpenConnectDtls12PskClient.cs#L21) |
| `DtlsPskParameters` | POCO tham số PSK hiện đại: `PskIdentity` (ASCII `"psk"`), `PskKey` (32B từ RFC 5705 EKM của CSTP TLS, label `EXPORTER-openconnect-psk`), `SessionId` (`X-DTLS-App-ID` decode hex → ClientHello session_id), `CipherSuites` (mặc định PSK-AES-256/128-GCM) | [DtlsPskParameters.cs:13](DtlsPskParameters.cs#L13) |
...
| RFC 4279 (TLS-PSK cipher suites) | `OpenConnectDtls12PskClient` (`PskTlsClient` + `BasicTlsPskIdentity`) | [OpenConnectDtls12PskClient.cs:21](OpenConnectDtls12PskClient.cs#L21) | OpenConnect `PSK-NEGOTIATE`: full DTLS 1.2 PSK handshake, identity `"psk"`, key 32B từ RFC 5705 EKM của CSTP TLS, suite `TLS_PSK_WITH_AES_256/128_GCM` (RFC 5487). VALIDATE LIVE ✅ ocserv 1.2.4 |
| RFC 5705 (TLS keying-material exporter) | nguồn PSK (ở `Transport.Tls.BouncyCastleTlsByteStream`) → `DtlsPskParameters.PskKey` | [DtlsPskParameters.cs:13](DtlsPskParameters.cs#L13) | PSK = exporter của CSTP TLS session, label `EXPORTER-openconnect-psk`, context rỗng, 32B (draft-mavrogiannopoulos-openconnect) |
