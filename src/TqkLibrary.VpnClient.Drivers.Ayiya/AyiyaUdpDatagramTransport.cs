...
{
    /// <summary>
    /// A connected UDP datagram pipe over a real socket, carrying an AYIYA tunnel's signed payload. <b>Passive</b> by
    ...
    /// <c>RawIpPassthroughChannel</c> behind an <see cref="AyiyaFramingTransport"/>) drives its own receive loop by calling
    /// <see cref="ReceiveAsync"/>. Supports both IPv4 and IPv6 by following the remote endpoint's <see cref="AddressFamily"/>.
    /// Mirrors the FOU driver's <c>FouUdpDatagramTransport</c>.
    /// </summary>
    internal sealed class AyiyaUdpDatagramTransport : IDatagramTransport
    {
        ...
        public AyiyaUdpDatagramTransport(IPEndPoint remote, IPAddress? localBind = null)
        {
        ...
        static ArraySegment<byte> GetArraySegment(Memory<byte> memory)
        {
            ...
            // (RawIpPassthroughChannel / AyiyaFramingTransport) always pass an array-backed buffer.
