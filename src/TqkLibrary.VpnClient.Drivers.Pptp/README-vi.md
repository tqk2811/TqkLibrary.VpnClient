...
2. **GRE data plane** — resolve server IP + local bind IP (`GetLocalAddress`, socket connected throwaway) → `_rawIpFactory.Create(serverIp, GreIpProtocol=47, localBind)` → [`new PptpGreChannel(greTransport, LocalCallId, PeerCallId, Logger)`](../TqkLibrary.VpnClient.Pptp/Gre/PptpGreChannel.cs#L43).
