...
{
    /// <summary>
    /// The production <see cref="IVxlanGpeTransportFactory"/>: opens a real connected UDP socket to the remote peer.
    /// VXLAN-GPE is UDP-only — one datagram is one message, no framing — so the transport is a thin
    ...
    /// an in-process factory. Mirrors <c>VxlanUdpTransportFactory</c>.
    /// </summary>
    public sealed class VxlanGpeUdpTransportFactory : IVxlanGpeTransportFactory
    {
        ...
        /// </summary>
        public VxlanGpeUdpTransportFactory(int receiveBufferSize = 65535, IPAddress? localBind = null)
        {
        ...
        /// <inheritdoc/>
        public async Task<VxlanGpeTransportHandle> ConnectAsync(IPEndPoint remote, CancellationToken cancellationToken)
        {
            ...
            return new VxlanGpeTransportHandle(socket, socket.SetReceiver, socket.RunReceiveLoopAsync);
        }

        /// <summary>A connected UDP datagram pipe over a real socket (live-only). Binds an ephemeral local port and
        /// connects to the remote peer; supports IPv4 and IPv6.</summary>
