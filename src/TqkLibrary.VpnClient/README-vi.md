...
- Cho ứng dụng một API gọn để **đăng ký các protocol driver** (SSTP, L2TP/IPsec, PPTP, IP-encap GRE/IPIP/SIT, GRE-in-UDP, FOU/GUE, VXLAN, VXLAN-GPE, Geneve, L2TPv3-eth, AYIYA, GTP-U, IKEv2, Cisco IPsec/EzVPN, OpenVPN, WireGuard, Nebula, Tailscale, tinc, n2n, vtun, VPN-over-SSH, ZeroTier, OpenConnect, SoftEther) theo kiểu fluent rồi **mở kết nối theo tên giao thức**.
...
- **Phụ thuộc (ProjectReference) — 25 project driver (không ref `Sockets`):**
...
Bản thân project façade **không hiện thực chuẩn mạng nào** — nó chỉ điều phối driver. Các chuẩn dưới đây được **truy cập gián tiếp** qua các shortcut `UseSstp()`/`UseL2tpIpsec()`/`UsePptp()`/`UseIpEncap()`/`UseGreInUdp()`/`UseFou()`/`UseGue()`/`UseIkev2()`/`UseCiscoIpsec()`/`UseOpenVpn()`/`UseWireGuard()`/`UseNebula()`/`UseTailscale()`/`UseTinc()`/`UseN2n()`/`UseVtun()`/`UseSsh()`/`UseZeroTier()`/`UseOpenConnect()`/`UseSoftEther()`/`UseVxlan()`/`UseVxlanGpe()`/`UseGeneve()`/`UseL2tpv3Ethernet()`/`UseAyiya()`/`UseGtpU()`; cột "Vị trí" link tới nơi façade kích hoạt driver tương ứng, và (nếu có) tới class summary ở driver. Project này không có comment `RFC` nào, nên gần như toàn bộ là **(suy luận)** — chi tiết ánh xạ chuẩn → code nằm ở README của các project Drivers/Ipsec/L2tp/Ppp/OpenVpn/WireGuard/SoftEther/Transport.Dtls.

| Chuẩn (RFC/FIPS/NIST/MS-*) | Class/Namespace áp dụng | Vị trí (link code) | Ghi chú |
| --- | --- | --- | --- |
| [MS-SSTP] (Secure Socket Tunneling Protocol) | `SstpDriver` qua `UseSstp()` | [VpnClientBuilder.cs:61-72](VpnClientBuilder.cs#L61-L72), [SstpDriver.cs:11](../TqkLibrary.VpnClient.Drivers.Sstp/SstpDriver.cs#L11) | Driver mô tả "TLS over 443, PPP, MS-CHAPv2"; cert TLS validate qua callback tùy chọn (P0.6) |
| RFC 2759 (MS-CHAPv2) | `SstpDriver` + `L2tpIpsecDriver` + `PptpDriver` (PPP auth) | [VpnClientBuilder.cs:61](VpnClientBuilder.cs#L61), [VpnClientBuilder.cs:75](VpnClientBuilder.cs#L75), [VpnClientBuilder.cs:103-105](VpnClientBuilder.cs#L103-L105) | (suy luận) — codec ở Crypto/`MsChapV2`, framing CHAP ở Ppp/`MsChapV2Authenticator` |
| RFC 2409 (IKEv1 / ISAKMP) | `L2tpIpsecDriver` qua `UseL2tpIpsec()` + `CiscoIpsecDriver` qua `UseCiscoIpsec()` (Aggressive Mode) | [VpnClientBuilder.cs:75-92](VpnClientBuilder.cs#L75-L92), [L2tpIpsecDriver.cs:12](../TqkLibrary.VpnClient.Drivers.L2tpIpsec/L2tpIpsecDriver.cs#L12), [CiscoIpsecDriver.cs:18](../TqkLibrary.VpnClient.Drivers.CiscoIpsec/CiscoIpsecDriver.cs#L18) | (suy luận) — comment driver "IKEv1 PSK over NAT-T"; logic ở Ipsec `Ike/V1` |
| RFC 7296 (IKEv2) + ESP tunnel mode + CP | `Ikev2Driver` qua `UseIkev2()` / `UseIkev2(IkeCertificateTrust)` | [VpnClientBuilder.cs:167-180](VpnClientBuilder.cs#L167-L180), [Ikev2Driver.cs:11](../TqkLibrary.VpnClient.Drivers.Ikev2/Ikev2Driver.cs#L11) | (suy luận) — driver "RFC 7296 PSK over NAT-T, CP virtual IP, ESP tunnel mode — no PPP"; logic ở Ipsec `Ike/V2` + `Esp` |
...
| RFC 2637 (PPTP) + RFC 3078/3079 (MPPE) | `PptpDriver` qua `UsePptp()` | [VpnClientBuilder.cs:103-105](VpnClientBuilder.cs#L103-L105), [PptpDriver.cs:16](../TqkLibrary.VpnClient.Drivers.Pptp/PptpDriver.cs#L16) | (suy luận) — TCP/1723 control + GRE proto-47 data + MPPE/RC4 + PPP/MS-CHAPv2; **legacy/không an toàn — interop only** |
| RFC 2784/2890 (GRE), RFC 2003 (IPIP), RFC 4213 (SIT/6in4) | `IpEncapDriver` qua `UseIpEncap()` (+ data plane GRE của `PptpDriver`) | [VpnClientBuilder.cs:117-119](VpnClientBuilder.cs#L117-L119), [IpEncapDriver.cs:20](../TqkLibrary.VpnClient.Drivers.IpEncap/IpEncapDriver.cs#L20) | (suy luận) — tunnel IP-in-IP **không mã hóa**, không control plane; raw IP proto-47/4/41 |
| RFC 8086 (GRE-in-UDP) + RFC 2784/2890 (GRE) | `GreInUdpDriver` qua `UseGreInUdp()` | [VpnClientBuilder.cs:122-133](VpnClientBuilder.cs#L122-L133), [GreInUdpDriver.cs:19](../TqkLibrary.VpnClient.Drivers.GreInUdp/GreInUdpDriver.cs#L19) | (suy luận) — header GRE trong payload **UDP/4754** thay raw-IP proto-47 — **không mã hóa**, **không elevate/raw socket**, qua NAT; tái dùng `GreCodec` của IpEncap |
| draft-ietf-intarea-gue (GUE) + FOU (Linux Foo-over-UDP) + RFC 2784/2890 (GRE) / RFC 2003 (IPIP) / RFC 4213 (SIT) | `FouDriver` qua `UseFou()` / `UseGue()` | [VpnClientBuilder.cs:135-164](VpnClientBuilder.cs#L135-L164), [FouDriver.cs:21](../TqkLibrary.VpnClient.Drivers.Fou/FouDriver.cs#L21) | (suy luận) — inner IP-proto (IPIP-4/41 hoặc GRE-47) trong payload **UDP**: FOU không header (proto suy từ port), GUE thêm header variant-0 4B mang Proto (skip ext theo Hlen, drop C-bit); tổng quát hoá GRE-in-UDP; **không mã hóa**, **không elevate/raw socket**; tái dùng `GreCodec`/`RawIpPassthroughChannel` của IpEncap |
...
| OpenVPN (community-server protocol) + NCP AEAD | `OpenVpnDriver` qua `UseOpenVpn()` | [VpnClientBuilder.cs:195-208](VpnClientBuilder.cs#L195-L208), [OpenVpnDriver.cs:24](../TqkLibrary.VpnClient.Drivers.OpenVpn/OpenVpnDriver.cs#L24) | (suy luận) — UDP/TCP, tun-mode (L3) / tap-mode (L2), tls-auth/tls-crypt; logic ở OpenVpn |
| WireGuard (Noise_IKpsk2 + ChaCha20-Poly1305) | `WireGuardDriver` qua `UseWireGuard()` (+ data plane của `TailscaleDriver`) | [VpnClientBuilder.cs:211-215](VpnClientBuilder.cs#L211-L215), [WireGuardDriver.cs:17](../TqkLibrary.VpnClient.Drivers.WireGuard/WireGuardDriver.cs#L17) | (suy luận) — UDP-only, static point-to-point, `SecurityKinds = Noise`, `AddressAssignment.OutOfBand`; logic ở WireGuard |
| Nebula (Noise_IX_25519_AESGCM_SHA256 + AES-256-GCM) | `NebulaDriver` qua `UseNebula()` | [VpnClientBuilder.cs:223-227](VpnClientBuilder.cs#L223-L227), [NebulaDriver.cs:17](../TqkLibrary.VpnClient.Drivers.Nebula/NebulaDriver.cs#L17) | (suy luận) — UDP, certificate auth chống network CA, data plane type-1 (Message); logic ở Nebula |
| Tailscale ts2021 (Noise IK control + WireGuard data) | `TailscaleDriver` qua `UseTailscale()` | [VpnClientBuilder.cs:238-242](VpnClientBuilder.cs#L238-L242), [TailscaleDriver.cs:19](../TqkLibrary.VpnClient.Drivers.Tailscale/TailscaleDriver.cs#L19) | (suy luận) — control plane Noise IK → Headscale/Tailscale + netmap, data plane WireGuard multi-peer; DERP/disco là future work |
| tinc 1.1 SPTPS (Curve25519/Ed25519/ChaCha-Poly1305) | `TincDriver` qua `UseTinc()` | [VpnClientBuilder.cs:251-255](VpnClientBuilder.cs#L251-L255), [TincDriver.cs:17](../TqkLibrary.VpnClient.Drivers.Tinc/TincDriver.cs#L17) | (suy luận) — TCP meta-connection + per-tunnel SPTPS + bare-IP router-mode over UDP; logic ở Tinc |
| n2n v3 (ntop: NULL / AES-CBC transform) | `N2nDriver` qua `UseN2n()` | [VpnClientBuilder.cs:265-269](VpnClientBuilder.cs#L265-L269), [N2nDriver.cs:17](../TqkLibrary.VpnClient.Drivers.N2n/N2nDriver.cs#L17) | (suy luận) — UDP tới supernode (REGISTER_SUPER), PACKET Ethernet frames, L2 bridge; logic ở N2n |
| vtun (legacy: MD5-keyed Blowfish-ECB challenge) | `VtunDriver` qua `UseVtun()` | [VpnClientBuilder.cs:280-284](VpnClientBuilder.cs#L280-L284), [VtunDriver.cs:18](../TqkLibrary.VpnClient.Drivers.Vtun/VtunDriver.cs#L18) | (suy luận) — 1 TCP, challenge-response, bare IP length-prefixed, ECHO keepalive; **crypto legacy/yếu — interop only** |
| SSH-2 transport (RFC 4251/4253) + `tun@openssh.com` | `SshDriver` qua `UseSsh()` | [VpnClientBuilder.cs:296-300](VpnClientBuilder.cs#L296-L300), [SshDriver.cs:18](../TqkLibrary.VpnClient.Drivers.Ssh/SshDriver.cs#L18) | (suy luận) — curve25519-sha256 KEX + ed25519 host-key + publickey/password, point-to-point L3 tun, bare IP; logic ở Ssh |
| ZeroTier VL1/VL2 (Curve25519 + Salsa20/12 + Poly1305) | `ZeroTierDriver` qua `UseZeroTier()` | [VpnClientBuilder.cs:313-317](VpnClientBuilder.cs#L313-L317), [ZeroTierDriver.cs:18](../TqkLibrary.VpnClient.Drivers.ZeroTier/ZeroTierDriver.cs#L18) | (suy luận) — UDP HELLO⇄OK identity agreement, NETWORK_CONFIG_REQUEST, EXT_FRAME L2 bridge; logic ở ZeroTier |
| OpenConnect (Cisco AnyConnect / ocserv CSTP) + RFC 6347 (DTLS 1.2) | `OpenConnectDriver` qua `UseOpenConnect()` | [VpnClientBuilder.cs:324-336](VpnClientBuilder.cs#L324-L336), [OpenConnectDriver.cs:20](../TqkLibrary.VpnClient.Drivers.OpenConnect/OpenConnectDriver.cs#L20) | (suy luận) — HTTPS config-auth → CSTP, bare IP (no PPP), `X-CSTP-DPD`; data plane DTLS 1.2 (V5.c) fallback TLS — DTLS ở [Transport.Dtls](../TqkLibrary.VpnClient.Transport.Dtls) |
| SoftEther SSL-VPN (Ethernet-over-TLS) | `SoftEtherDriver` qua `UseSoftEther()` | [VpnClientBuilder.cs:342-357](VpnClientBuilder.cs#L342-L357), [SoftEtherDriver.cs:23](../TqkLibrary.VpnClient.Drivers.SoftEther/SoftEtherDriver.cs#L23) | (suy luận) — L2 segment, DHCP-leased IP (`AddressAssignment.Dhcp`), SHA-0 password auth; logic ở SoftEther |
| RFC 7348 (VXLAN) | `VxlanDriver` qua `UseVxlan()` | [VpnClientBuilder.cs:359-371](VpnClientBuilder.cs#L359-L371), [VxlanDriver.cs:20](../TqkLibrary.VpnClient.Drivers.Vxlan/VxlanDriver.cs#L20) | (suy luận) — L2-over-UDP/**4789**, VXLAN header 8B (flags 0x08 + VNI 24-bit) → Ethernet L2 fabric, static unicast remote VTEP; **không mã hóa**, **không control plane**, **không elevate/raw socket**; logic ở Vxlan |
| draft-ietf-nvo3-vxlan-gpe (VXLAN-GPE) | `VxlanGpeDriver` qua `UseVxlanGpe()` | [VpnClientBuilder.cs:373-388](VpnClientBuilder.cs#L373-L388), [VxlanGpeDriver.cs:21](../TqkLibrary.VpnClient.Drivers.VxlanGpe/VxlanGpeDriver.cs#L21) | (suy luận + clean-room từ draft) — L2-over-UDP/**4790**, header VXLAN-GPE 8B (flags I\|P `0x0C` + Next-Protocol byte3 + VNI 24-bit) — **superset VXLAN** thêm bit P + Next-Protocol (0x01 IPv4/0x02 IPv6/**0x03 Ethernet**/0x04 NSH) → Ethernet L2 fabric, static unicast remote; ingress drop Ver≠0/thiếu I\|P/OAM/Next-Protocol≠Ethernet; **chỉ Ethernet đã wire** (IPv4/IPv6/NSH parse/validate); **không mã hóa**, **không control plane**, **không elevate/raw socket**; logic ở VxlanGpe |
| RFC 8926 (Geneve) | `GeneveDriver` qua `UseGeneve()` | [VpnClientBuilder.cs:390-405](VpnClientBuilder.cs#L390-L405), [GeneveDriver.cs:21](../TqkLibrary.VpnClient.Drivers.Geneve/GeneveDriver.cs#L21) | (suy luận) — L2-over-UDP/**6081**, base header 8B (Ver/OptLen + proto-type 0x6558 + VNI 24-bit) + options TLV biến thiên → Ethernet L2 fabric, static unicast remote; receiver skip options theo OptLen + drop critical-option lạ (§3.5); sibling VXLAN; **không mã hóa**, **không control plane**, **không elevate/raw socket**; logic ở Geneve |
| RFC 3931 (L2TPv3) + RFC 4719 (Ethernet PW) | `L2tpv3EthDriver` qua `UseL2tpv3Ethernet()` | [VpnClientBuilder.cs:407-424](VpnClientBuilder.cs#L407-L424), [L2tpv3EthDriver.cs:22](../TqkLibrary.VpnClient.Drivers.L2tpv3Eth/L2tpv3EthDriver.cs#L22) | (suy luận) — L2-over-UDP/**1701** static/unmanaged, header data L2TPv3 (Session ID 32-bit + Cookie 0/4/8 + Default L2-Specific Sublayer 4B tùy chọn) → Ethernet L2 fabric, static unicast remote; Session ID + Cookie tĩnh 2 đầu, **không control plane**; ingress drop control/zero-SID/mismatch/cookie/truncated; chạy header L2TPv3-over-IP thẳng trong UDP (no-admin, KHÁC Linux `encap udp` prefix 4B); sibling VXLAN/Geneve; **không mã hóa**, **không elevate/raw socket**; logic ở L2tpv3Eth |
| draft-massar-v6ops-ayiya-02 (AYIYA) + shared-secret SHA-1 sig (aiccu) + RFC 3053 (TIC — bỏ qua ở static mode) | `AyiyaDriver` qua `UseAyiya()` | [VpnClientBuilder.cs:440-445](VpnClientBuilder.cs#L440-L445), [AyiyaDriver.cs:21](../TqkLibrary.VpnClient.Drivers.Ayiya/AyiyaDriver.cs#L21) | (suy luận + clean-room từ draft) — tunnel-broker IPv6-over-UDP/**5072** **static mode** (bỏ control-channel TIC): gói IPv6 sau header AYIYA (header 4B idlen/idtype‖siglen/hshmeth‖autmeth/opcode‖nextheader + epoch time 4B BE + identity 2^idlen + signature siglen×4), **ký toàn vẹn shared-secret SHA-1** (đặt SHA1(password) vào ô signature → hash toàn gói → ghi đè) + **chống replay bằng epoch time**; tái dùng `RawIpPassthroughChannel` (IPv6) của IpEncap qua decorator ký/kiểm; ⚠️ **KHÔNG mã hóa payload**, **không control-plane động**, **không elevate/raw socket**; logic ở Drivers.Ayiya (SHA-1 dùng thẳng BCL) |
| 3GPP TS 29.281 (GTP-U) | `GtpUDriver` qua `UseGtpU()` | [VpnClientBuilder.cs:447-461](VpnClientBuilder.cs#L447-L461), [GtpUDriver.cs:21](../TqkLibrary.VpnClient.Drivers.GtpU/GtpUDriver.cs#L21) | (suy luận + clean-room từ TS 29.281) — tunnel user-plane inner-IP-over-UDP/**2152** **static mode** (không GTP-C): gói IP sau header G-PDU 8B (Version 1 \| PT \| E/S/PN ‖ message type 255 ‖ Length ‖ TEID 32-bit BE) gắn `Teid`, optional Sequence Number, extension header skip theo length; tái dùng `RawIpPassthroughChannel` của IpEncap qua decorator framing; ingress drop non-version-1 / non-G-PDU (Echo/Error-Indication) / TEID-lệch / runt / length-sai; ⚠️ **KHÔNG mã hóa/xác thực** (chỉ gắn TEID), **không control plane**, **không elevate/raw socket**; logic ở Drivers.GtpU |
...
1. **Đăng ký (build-time).** Các shortcut `UseSstp()`/`UseL2tpIpsec()`/`UsePptp()`/`UseIpEncap()`/`UseGreInUdp()`/`UseFou()`/`UseGue()`/`UseIkev2()`/`UseCiscoIpsec()`/`UseOpenVpn()`/`UseWireGuard()`/`UseNebula()`/`UseTailscale()`/`UseTinc()`/`UseN2n()`/`UseVtun()`/`UseSsh()`/`UseZeroTier()`/`UseOpenConnect()`/`UseSoftEther()`/`UseVxlan()`/`UseVxlanGpe()`/`UseGeneve()`/`UseL2tpv3Ethernet()`/`UseAyiya()`/`UseGtpU()` tạo driver cụ thể và gọi `AddDriver` để nạp vào `Dictionary<string, IVpnProtocolDriver>` keyed theo `driver.Name` (`"sstp"`, `"l2tp-ipsec"`, `"pptp"`, `"ipencap"`, `"gre-udp"`, `"fou"`, `"gue"`, `"ikev2"`, `"cisco-ipsec"`, `"openvpn"`, `"wireguard"`, `"nebula"`, `"tailscale"`, `"tinc"`, `"n2n"`, `"vtun"`, `"ssh"`, `"zerotier"`, `"openconnect"`, `"softether"`, `"vxlan"`, `"vxlan-gpe"`, `"geneve"`, `"l2tpv3-eth"`, `"ayiya"`, `"gtpu"`) — [VpnClientBuilder.cs:54-465](VpnClientBuilder.cs#L54-L465). `Build()` đóng gói dictionary đó vào `VpnClient` — [VpnClientBuilder.cs:465](VpnClientBuilder.cs#L465).
