...
- Cho ứng dụng một API gọn để **đăng ký các protocol driver** (SSTP, L2TP/IPsec, PPTP, IP-encap GRE/IPIP/SIT, GRE-in-UDP, FOU/GUE, VXLAN, VXLAN-GPE, Geneve, L2TPv3-eth, AYIYA, GTP-U, EoGRE/NVGRE, IKEv2, Cisco IPsec/EzVPN, OpenVPN, WireGuard, Nebula, Tailscale, tinc, n2n, vtun, VPN-over-SSH, ZeroTier, OpenConnect, SoftEther) theo kiểu fluent rồi **mở kết nối theo tên giao thức**.
...
- Là nơi **đảo ngược phụ thuộc** hội tụ: façade biết về các driver cụ thể (`SstpDriver`, `L2tpIpsecDriver`, `PptpDriver`, `IpEncapDriver`, `GreInUdpDriver`, `FouDriver`, `VxlanDriver`, `VxlanGpeDriver`, `GeneveDriver`, `L2tpv3EthDriver`, `AyiyaDriver`, `GtpUDriver`, `Ikev2Driver`, `CiscoIpsecDriver`, `OpenVpnDriver`, `WireGuardDriver`, `NebulaDriver`, `TailscaleDriver`, `TincDriver`, `N2nDriver`, `VtunDriver`, `SshDriver`, `ZeroTierDriver`, `OpenConnectDriver`, `SoftEtherDriver`), còn mọi tầng dưới chỉ phụ thuộc vào abstractions.
...
- **Phụ thuộc (ProjectReference) — 26 project driver (không ref `Sockets`):**
  ...
  - [TqkLibrary.VpnClient.Drivers.GtpU](../TqkLibrary.VpnClient.Drivers.GtpU) — nơi `GtpUDriver` được khai báo (GTP-U static-mode inner-IP-over-UDP/2152).
  ...
  - [TqkLibrary.VpnClient.Drivers.EoGre](../TqkLibrary.VpnClient.Drivers.EoGre) — nơi `EoGreDriver` được khai báo (đăng ký cả `eogre` lẫn `nvgre`).
...
```
TqkLibrary.VpnClient/
├── VpnClientBuilder.cs   # Builder fluent: AddDriver / UseSstp / UseL2tpIpsec / UsePptp / UseIpEncap / UseGreInUdp / UseFou / UseGue / UseIkev2 / UseCiscoIpsec / UseOpenVpn / UseWireGuard / UseNebula / UseTailscale / UseTinc / UseN2n / UseVtun / UseSsh / UseZeroTier / UseOpenConnect / UseSoftEther / UseVxlan / UseVxlanGpe / UseGeneve / UseL2tpv3Ethernet / UseAyiya / UseGtpU / UseEoGre / UseNvgre → Build()
...
| Type | Vai trò | Vị trí |
| --- | --- | --- |
| `VpnClientBuilder` | Builder fluent: đăng ký driver theo `Name`, có 28 nhóm shortcut `UseSstp()`/`UseL2tpIpsec()`/`UsePptp()`/`UseIpEncap()`/`UseGreInUdp()`/`UseFou()`/`UseGue()`/`UseIkev2()`/`UseCiscoIpsec()`/`UseOpenVpn()`/`UseWireGuard()`/`UseNebula()`/`UseTailscale()`/`UseTinc()`/`UseN2n()`/`UseVtun()`/`UseSsh()`/`UseZeroTier()`/`UseOpenConnect()`/`UseSoftEther()`/`UseVxlan()`/`UseVxlanGpe()`/`UseGeneve()`/`UseL2tpv3Ethernet()`/`UseAyiya()`/`UseGtpU()`/`UseEoGre()`/`UseNvgre()`, kết thúc bằng `Build()` | [VpnClientBuilder.cs:52](VpnClientBuilder.cs#L52) |
| `VpnClientBuilder.AddDriver` | Đăng ký một `IVpnProtocolDriver` bất kỳ (keyed theo `driver.Name`) | [VpnClientBuilder.cs:57](VpnClientBuilder.cs#L57) |
| `VpnClientBuilder.UseSstp` | Đăng ký `SstpDriver` (key `"sstp"`), auto-reconnect bật mặc định; overload nhận `SstpReconnectOptions` và/hoặc `RemoteCertificateValidationCallback` (cert TLS, P0.6 — null ⇒ accept all) | [VpnClientBuilder.cs:64-75](VpnClientBuilder.cs#L64-L75) |
| `VpnClientBuilder.UseL2tpIpsec` | Đăng ký `L2tpIpsecDriver` (key `"l2tp-ipsec"`), auto-reconnect bật mặc định; overload nhận `L2tpIpsecReconnectOptions`, `(reconnect, L2tpIpsecTimeoutOptions)`, và `(IRawIpTransportFactory, L2tpIpsecNatTraversalMode, reconnect?, timeout?)` bật **native ESP** (proto-50, cần elevation) | [VpnClientBuilder.cs:78-95](VpnClientBuilder.cs#L78-L95) |
| `VpnClientBuilder.UsePptp` | Đăng ký `PptpDriver` (key `"pptp"`, RFC 2637: TCP/1723 control + GRE proto-47 data + MPPE/RC4 + PPP/MS-CHAPv2), cần `IRawIpTransportFactory` cho GRE; overload nhận `PptpReconnectOptions?` + `PptpTimeoutOptions?`. **Legacy/không an toàn — interop only** | [VpnClientBuilder.cs:106-108](VpnClientBuilder.cs#L106-L108) |
| `VpnClientBuilder.UseIpEncap` | Đăng ký `IpEncapDriver` (key `"ipencap"`, tunnel IP-in-IP: GRE proto-47 / IPIP proto-4 / SIT 6in4 proto-41 — **không mã hóa**, không control plane), cần `IRawIpTransportFactory`; overload nhận `IpEncapOptions?` + `IpEncapReconnectOptions?` | [VpnClientBuilder.cs:120-122](VpnClientBuilder.cs#L120-L122) |
| `VpnClientBuilder.UseGreInUdp` | Đăng ký `GreInUdpDriver` (key `"gre-udp"`, GRE-in-UDP RFC 8086: chở header GRE trong payload **UDP/4754** thay raw-IP proto-47 — **KHÔNG mã hóa**, **KHÔNG elevate/raw socket**, qua NAT; tái dùng `GreTunnelChannel`/`GreCodec` của `IpEncap`), host qua `VpnEndpoint.Host`; overload nhận `GreInUdpOptions?` (Port=4754/Mtu/Gre) + `GreInUdpReconnectOptions?` | [VpnClientBuilder.cs:125-136](VpnClientBuilder.cs#L125-L136) |
| `VpnClientBuilder.UseFou` | Đăng ký `FouDriver` (key `"fou"`, FOU Foo-over-UDP: chở payload inner IP-proto — IPIP proto-4/41 hoặc GRE proto-47 theo `FouOptions.InnerProtocol` — trong payload UDP **KHÔNG header** (proto suy từ port, như Linux `ip fou`); tổng quát hoá GRE-in-UDP; **KHÔNG mã hóa**, **KHÔNG elevate/raw socket**, qua NAT; tái dùng kênh data-plane GRE/IPIP của `IpEncap`), host qua `VpnEndpoint.Host`; overload nhận `FouOptions?` (Mode ép FOU + InnerProtocol/Port=6080/Mtu/Gre) + `FouReconnectOptions?` | [VpnClientBuilder.cs:138-152](VpnClientBuilder.cs#L138-L152) |
| `VpnClientBuilder.UseGue` | Đăng ký `FouDriver` (key `"gue"`, GUE draft-ietf-intarea-gue **variant-0**: như `UseFou` nhưng thêm header GUE 4B mang Proto (skip extension theo Hlen, drop control-bit C=1) ⇒ 1 UDP port multiplex nhiều inner-proto; **KHÔNG mã hóa**, **KHÔNG elevate/raw socket**, qua NAT), host qua `VpnEndpoint.Host`; overload nhận `FouOptions?` (Mode ép GUE) + `FouReconnectOptions?` | [VpnClientBuilder.cs:154-167](VpnClientBuilder.cs#L154-L167) |
| `VpnClientBuilder.UseIkev2` | Đăng ký `Ikev2Driver` (key `"ikev2"`, IKEv2-native RFC 7296 PSK/EAP + ESP tunnel mode), auto-reconnect bật mặc định; overload nhận `Ikev2ReconnectOptions`, và overload `UseIkev2(IkeCertificateTrust, Ikev2ReconnectOptions?)` verify gateway bằng **certificate** (chữ ký số RFC 7296 §2.15) | [VpnClientBuilder.cs:170-183](VpnClientBuilder.cs#L170-L183) |
| `VpnClientBuilder.UseCiscoIpsec` | Đăng ký `CiscoIpsecDriver` (key `"cisco-ipsec"`, IKEv1 Aggressive Mode group PSK + XAUTH + Mode-Config + ESP tunnel-mode, forced NAT-T, no PPP) cho group `groupName`, auto-reconnect bật mặc định; overload nhận `CiscoIpsecReconnectOptions?`. **Aggressive Mode + group PSK yếu — interop only** | [VpnClientBuilder.cs:194-195](VpnClientBuilder.cs#L194-L195) |
| `VpnClientBuilder.UseOpenVpn` | Đăng ký `OpenVpnDriver` (key `"openvpn"`) từ một `OpenVpnProfile` đã parse; overload nhận `X509CertificateCollection?` (client cert) + `RemoteCertificateValidationCallback?` (cert server) + `OpenVpnReconnectOptions?`, và overload `(OpenVpnProfile, bool enableIpv6)` bật IPv6 trong tunnel (chỉ tap-mode) | [VpnClientBuilder.cs:198-211](VpnClientBuilder.cs#L198-L211) |
| `VpnClientBuilder.UseWireGuard` | Đăng ký `WireGuardDriver` (key `"wireguard"`) từ một `WireGuardConfig` tĩnh, auto-reconnect bật mặc định; overload nhận `WireGuardReconnectOptions` | [VpnClientBuilder.cs:214-218](VpnClientBuilder.cs#L214-L218) |
| `VpnClientBuilder.UseNebula` | Đăng ký `NebulaDriver` (key `"nebula"`, Slack mesh: UDP + Noise_IX_25519_AESGCM_SHA256 cert-auth + AES-256-GCM type-1) từ một `NebulaConfig` tĩnh, auto-reconnect bật mặc định; overload nhận `NebulaReconnectOptions` | [VpnClientBuilder.cs:226-230](VpnClientBuilder.cs#L226-L230) |
| `VpnClientBuilder.UseTailscale` | Đăng ký `TailscaleDriver` (key `"tailscale"`, control plane ts2021 Noise IK → Headscale/Tailscale + netmap, data plane WireGuard multi-peer) từ một `TailscaleConfig` tĩnh, auto-reconnect bật mặc định; overload nhận `TailscaleReconnectOptions` | [VpnClientBuilder.cs:241-245](VpnClientBuilder.cs#L241-L245) |
| `VpnClientBuilder.UseTinc` | Đăng ký `TincDriver` (key `"tinc"`, tinc 1.1 SPTPS: TCP meta-connection + per-tunnel SPTPS + bare-IP router-mode over UDP) từ một `TincConfig` tĩnh, auto-reconnect bật mặc định; overload nhận `TincReconnectOptions` | [VpnClientBuilder.cs:254-258](VpnClientBuilder.cs#L254-L258) |
| `VpnClientBuilder.UseN2n` | Đăng ký `N2nDriver` (key `"n2n"`, n2n v3: UDP tới supernode, REGISTER_SUPER + PACKET Ethernet frames NULL/AES-CBC, L2 bridge) từ một `N2nConfig` tĩnh, auto-reconnect bật mặc định; overload nhận `N2nReconnectOptions` | [VpnClientBuilder.cs:268-272](VpnClientBuilder.cs#L268-L272) |
| `VpnClientBuilder.UseVtun` | Đăng ký `VtunDriver` (key `"vtun"`, vtund legacy: 1 TCP, challenge-response MD5/Blowfish-ECB, bare IP length-prefixed, ECHO keepalive) từ một `VtunConfig` tĩnh, auto-reconnect bật mặc định; overload nhận `VtunReconnectOptions`. **Crypto legacy/yếu — interop only** | [VpnClientBuilder.cs:283-287](VpnClientBuilder.cs#L283-L287) |
| `VpnClientBuilder.UseSsh` | Đăng ký `SshDriver` (key `"ssh"`, VPN-over-SSH OpenSSH `-w` tun: SSH-2 KEX curve25519 + ed25519 host-key + publickey/password, `tun@openssh.com` L3, bare IP) từ một `SshConfig` tĩnh, auto-reconnect bật mặc định; overload nhận `SshReconnectOptions` | [VpnClientBuilder.cs:299-303](VpnClientBuilder.cs#L299-L303) |
| `VpnClientBuilder.UseZeroTier` | Đăng ký `ZeroTierDriver` (key `"zerotier"`, VL1/VL2: UDP HELLO⇄OK Curve25519 + Salsa20/12+Poly1305, NETWORK_CONFIG_REQUEST, EXT_FRAME L2 bridge) từ một `ZeroTierConfig` tĩnh, auto-reconnect bật mặc định; overload nhận `ZeroTierReconnectOptions` | [VpnClientBuilder.cs:316-320](VpnClientBuilder.cs#L316-L320) |
| `VpnClientBuilder.UseOpenConnect` | Đăng ký `OpenConnectDriver` (key `"openconnect"`, Cisco AnyConnect/ocserv: HTTPS config-auth → CSTP, bare IP, DTLS 1.2 data plane fallback TLS), auto-reconnect bật mặc định; overload nhận `OpenConnectReconnectOptions`, và `(reconnect, RemoteCertificateValidationCallback?, groupSelect)` | [VpnClientBuilder.cs:327-339](VpnClientBuilder.cs#L327-L339) |
| `VpnClientBuilder.UseSoftEther` | Đăng ký `SoftEtherDriver` (key `"softether"`, SSL-VPN Ethernet-over-TLS, DHCP-leased, SHA-0 password auth) targeting `hubName`, auto-reconnect bật mặc định; overload nhận `SoftEtherSessionParams`, `(session, SoftEtherReconnectOptions)`, và `(hubName, bool enableIpv6)` | [VpnClientBuilder.cs:345-360](VpnClientBuilder.cs#L345-L360) |
| `VpnClientBuilder.UseVxlan` | Đăng ký `VxlanDriver` (key `"vxlan"`, VXLAN RFC 7348: L2-over-UDP/**4789**, VXLAN header 8B + VNI 24-bit → Ethernet L2 fabric; **KHÔNG mã hóa**, **KHÔNG elevate/raw socket**), host qua `VpnEndpoint.Host`; overload nhận `VxlanReconnectOptions` | [VpnClientBuilder.cs:362-374](VpnClientBuilder.cs#L362-L374) |
| `VpnClientBuilder.UseVxlanGpe` | Đăng ký `VxlanGpeDriver` (key `"vxlan-gpe"`, VXLAN-GPE `draft-ietf-nvo3-vxlan-gpe`: L2-over-UDP/**4790**, header VXLAN-GPE 8B (flags I\|P `0x0C` + Next-Protocol byte3 + VNI 24-bit BE) — **superset VXLAN** thêm bit P + Next-Protocol (0x01 IPv4/0x02 IPv6/**0x03 Ethernet**/0x04 NSH) → Ethernet L2 fabric; ingress drop nếu Ver≠0/thiếu I\|P/OAM/Next-Protocol≠Ethernet; **chỉ Ethernet đã wire data plane** (IPv4/IPv6/NSH chỉ parse/validate); **KHÔNG mã hóa**, **KHÔNG control plane**, **KHÔNG elevate/raw socket**), host qua `VpnEndpoint.Host`; overload nhận `VxlanGpeReconnectOptions` | [VpnClientBuilder.cs:376-391](VpnClientBuilder.cs#L376-L391) |
| `VpnClientBuilder.UseGeneve` | Đăng ký `GeneveDriver` (key `"geneve"`, Geneve RFC 8926: L2-over-UDP/**6081**, base header 8B + VNI 24-bit + options TLV biến thiên (proto-type 0x6558) → Ethernet L2 fabric; sibling VXLAN; receiver skip options theo OptLen + **drop khi có critical-option lạ** RFC 8926 §3.5; **KHÔNG mã hóa**, **KHÔNG elevate/raw socket**), host qua `VpnEndpoint.Host`; overload nhận `GeneveReconnectOptions` | [VpnClientBuilder.cs:393-408](VpnClientBuilder.cs#L393-L408) |
| `VpnClientBuilder.UseL2tpv3Ethernet` | Đăng ký `L2tpv3EthDriver` (key `"l2tpv3-eth"`, L2TPv3 RFC 3931 + RFC 4719 Ethernet-pseudowire **static/unmanaged**: L2-over-UDP/**1701**, header data L2TPv3 (Session ID 32-bit + Cookie 0/4/8 + Default L2-Specific Sublayer 4B tùy chọn cho sequencing) → Ethernet L2 fabric; sibling VXLAN/Geneve, **KHÔNG control plane** (Session ID + Cookie cấu hình tĩnh 2 đầu); ingress drop control-message/zero-SID/SID-mismatch/cookie-mismatch/truncated; chạy header L2TPv3-over-IP thẳng trong UDP payload (biến thể no-admin, KHÁC Linux `encap udp` có prefix 4B); **KHÔNG mã hóa**, **KHÔNG elevate/raw socket**), host qua `VpnEndpoint.Host`; overload nhận `L2tpv3EthReconnectOptions` | [VpnClientBuilder.cs:410-427](VpnClientBuilder.cs#L410-L427) |
| `VpnClientBuilder.UseAyiya` | Đăng ký `AyiyaDriver` (key `"ayiya"`, AYIYA `draft-massar-v6ops-ayiya-02` tunnel-broker IPv6-over-UDP/**5072** **static mode** — bỏ qua control-channel TIC: chở gói IPv6 sau header AYIYA (header 4B + epoch time + identity + signature), **ký shared-secret SHA-1** (thuật toán aiccu) + **chống replay bằng epoch time**, tái dùng `RawIpPassthroughChannel` (IPv6) của IpEncap qua decorator ký/kiểm; **KHÔNG mã hóa payload** (chỉ ký toàn vẹn + chống replay), **KHÔNG elevate/raw socket**, qua NAT), host qua `VpnEndpoint.Host`; nhận `AyiyaOptions` (Identity+Password bắt buộc); overload nhận `AyiyaReconnectOptions` | [VpnClientBuilder.cs:443-448](VpnClientBuilder.cs#L443-L448) |
| `VpnClientBuilder.UseGtpU` | Đăng ký `GtpUDriver` (key `"gtpu"`, GTP-U 3GPP TS 29.281 **static mode** — không GTP-C: chở gói inner IP trong payload **UDP/2152** sau header G-PDU 8B (message type 255) gắn `Teid` 32-bit (tùy chọn Sequence Number 16-bit; extension header skip khi decode), tái dùng `RawIpPassthroughChannel` của IpEncap qua decorator framing; **KHÔNG mã hóa/không xác thực** — chỉ gắn TEID; **KHÔNG control plane**, **KHÔNG elevate/raw socket**, qua NAT), host qua `VpnEndpoint.Host`; overload nhận `GtpUOptions?` (Port=2152/Teid/ExpectedInboundTeid?/EnableSequence/Mtu) + `GtpUReconnectOptions?` | [VpnClientBuilder.cs:450-464](VpnClientBuilder.cs#L450-L464) |
| `VpnClientBuilder.UseEoGre` | Đăng ký `EoGreDriver` (key `"eogre"`, EoGRE/GRETAP RFC 8086 + RFC 2784/2890: L2-over-UDP/**4754**, header GRE (proto-type 0x6558) trong payload UDP chở Ethernet frame → Ethernet L2 fabric; VSID 24-bit tùy chọn trong GRE Key RFC 2890, Checksum/Sequence tùy chọn; **tái dùng `GreCodec`/`GrePacket` của IpEncap**; **KHÔNG mã hóa**, **KHÔNG control plane**, **KHÔNG elevate/raw socket**), host qua `VpnEndpoint.Host`; overload nhận `EoGreReconnectOptions` | [VpnClientBuilder.cs:478-481](VpnClientBuilder.cs#L478-L481) |
| `VpnClientBuilder.UseNvgre` | Đăng ký `EoGreDriver` (key `"nvgre"`, NVGRE RFC 7637: như EoGRE nhưng **VSID 24-bit bắt buộc** + FlowID 8-bit trong GRE Key, Checksum/Sequence ép tắt; ingress drop VSID-mismatch), host qua `VpnEndpoint.Host`; overload nhận `EoGreReconnectOptions` | [VpnClientBuilder.cs:492-495](VpnClientBuilder.cs#L492-L495) |
...
Bản thân project façade **không hiện thực chuẩn mạng nào** — nó chỉ điều phối driver. Các chuẩn dưới đây được **truy cập gián tiếp** qua các shortcut `UseSstp()`/`UseL2tpIpsec()`/`UsePptp()`/`UseIpEncap()`/`UseGreInUdp()`/`UseFou()`/`UseGue()`/`UseIkev2()`/`UseCiscoIpsec()`/`UseOpenVpn()`/`UseWireGuard()`/`UseNebula()`/`UseTailscale()`/`UseTinc()`/`UseN2n()`/`UseVtun()`/`UseSsh()`/`UseZeroTier()`/`UseOpenConnect()`/`UseSoftEther()`/`UseVxlan()`/`UseVxlanGpe()`/`UseGeneve()`/`UseL2tpv3Ethernet()`/`UseAyiya()`/`UseGtpU()`/`UseEoGre()`/`UseNvgre()`; cột "Vị trí" link tới nơi façade kích hoạt driver tương ứng, và (nếu có) tới class summary ở driver. Project này không có comment `RFC` nào, nên gần như toàn bộ là **(suy luận)** — chi tiết ánh xạ chuẩn → code nằm ở README của các project Drivers/Ipsec/L2tp/Ppp/OpenVpn/WireGuard/SoftEther/Transport.Dtls.

| Chuẩn (RFC/FIPS/NIST/MS-*) | Class/Namespace áp dụng | Vị trí (link code) | Ghi chú |
| --- | --- | --- | --- |
| [MS-SSTP] (Secure Socket Tunneling Protocol) | `SstpDriver` qua `UseSstp()` | [VpnClientBuilder.cs:64-75](VpnClientBuilder.cs#L64-L75), [SstpDriver.cs:11](../TqkLibrary.VpnClient.Drivers.Sstp/SstpDriver.cs#L11) | Driver mô tả "TLS over 443, PPP, MS-CHAPv2"; cert TLS validate qua callback tùy chọn (P0.6) |
| RFC 2759 (MS-CHAPv2) | `SstpDriver` + `L2tpIpsecDriver` + `PptpDriver` (PPP auth) | [VpnClientBuilder.cs:64](VpnClientBuilder.cs#L64), [VpnClientBuilder.cs:78](VpnClientBuilder.cs#L78), [VpnClientBuilder.cs:106-108](VpnClientBuilder.cs#L106-L108) | (suy luận) — codec ở Crypto/`MsChapV2`, framing CHAP ở Ppp/`MsChapV2Authenticator` |
| RFC 2409 (IKEv1 / ISAKMP) | `L2tpIpsecDriver` qua `UseL2tpIpsec()` + `CiscoIpsecDriver` qua `UseCiscoIpsec()` (Aggressive Mode) | [VpnClientBuilder.cs:78-95](VpnClientBuilder.cs#L78-L95), [L2tpIpsecDriver.cs:12](../TqkLibrary.VpnClient.Drivers.L2tpIpsec/L2tpIpsecDriver.cs#L12), [CiscoIpsecDriver.cs:18](../TqkLibrary.VpnClient.Drivers.CiscoIpsec/CiscoIpsecDriver.cs#L18) | (suy luận) — comment driver "IKEv1 PSK over NAT-T"; logic ở Ipsec `Ike/V1` |
| RFC 7296 (IKEv2) + ESP tunnel mode + CP | `Ikev2Driver` qua `UseIkev2()` / `UseIkev2(IkeCertificateTrust)` | [VpnClientBuilder.cs:170-183](VpnClientBuilder.cs#L170-L183), [Ikev2Driver.cs:11](../TqkLibrary.VpnClient.Drivers.Ikev2/Ikev2Driver.cs#L11) | (suy luận) — driver "RFC 7296 PSK over NAT-T, CP virtual IP, ESP tunnel mode — no PPP"; logic ở Ipsec `Ike/V2` + `Esp` |
...
| RFC 2637 (PPTP) + RFC 3078/3079 (MPPE) | `PptpDriver` qua `UsePptp()` | [VpnClientBuilder.cs:106-108](VpnClientBuilder.cs#L106-L108), [PptpDriver.cs:16](../TqkLibrary.VpnClient.Drivers.Pptp/PptpDriver.cs#L16) | (suy luận) — TCP/1723 control + GRE proto-47 data + MPPE/RC4 + PPP/MS-CHAPv2; **legacy/không an toàn — interop only** |
| RFC 2784/2890 (GRE), RFC 2003 (IPIP), RFC 4213 (SIT/6in4) | `IpEncapDriver` qua `UseIpEncap()` (+ data plane GRE của `PptpDriver`) | [VpnClientBuilder.cs:120-122](VpnClientBuilder.cs#L120-L122), [IpEncapDriver.cs:20](../TqkLibrary.VpnClient.Drivers.IpEncap/IpEncapDriver.cs#L20) | (suy luận) — tunnel IP-in-IP **không mã hóa**, không control plane; raw IP proto-47/4/41 |
| RFC 8086 (GRE-in-UDP) + RFC 2784/2890 (GRE) | `GreInUdpDriver` qua `UseGreInUdp()` | [VpnClientBuilder.cs:125-136](VpnClientBuilder.cs#L125-L136), [GreInUdpDriver.cs:19](../TqkLibrary.VpnClient.Drivers.GreInUdp/GreInUdpDriver.cs#L19) | (suy luận) — header GRE trong payload **UDP/4754** thay raw-IP proto-47 — **không mã hóa**, **không elevate/raw socket**, qua NAT; tái dùng `GreCodec` của IpEncap |
| draft-ietf-intarea-gue (GUE) + FOU (Linux Foo-over-UDP) + RFC 2784/2890 (GRE) / RFC 2003 (IPIP) / RFC 4213 (SIT) | `FouDriver` qua `UseFou()` / `UseGue()` | [VpnClientBuilder.cs:138-167](VpnClientBuilder.cs#L138-L167), [FouDriver.cs:21](../TqkLibrary.VpnClient.Drivers.Fou/FouDriver.cs#L21) | (suy luận) — inner IP-proto (IPIP-4/41 hoặc GRE-47) trong payload **UDP**: FOU không header (proto suy từ port), GUE thêm header variant-0 4B mang Proto (skip ext theo Hlen, drop C-bit); tổng quát hoá GRE-in-UDP; **không mã hóa**, **không elevate/raw socket**; tái dùng `GreCodec`/`RawIpPassthroughChannel` của IpEncap |
...
| OpenVPN (community-server protocol) + NCP AEAD | `OpenVpnDriver` qua `UseOpenVpn()` | [VpnClientBuilder.cs:198-211](VpnClientBuilder.cs#L198-L211), [OpenVpnDriver.cs:24](../TqkLibrary.VpnClient.Drivers.OpenVpn/OpenVpnDriver.cs#L24) | (suy luận) — UDP/TCP, tun-mode (L3) / tap-mode (L2), tls-auth/tls-crypt; logic ở OpenVpn |
| WireGuard (Noise_IKpsk2 + ChaCha20-Poly1305) | `WireGuardDriver` qua `UseWireGuard()` (+ data plane của `TailscaleDriver`) | [VpnClientBuilder.cs:214-218](VpnClientBuilder.cs#L214-L218), [WireGuardDriver.cs:17](../TqkLibrary.VpnClient.Drivers.WireGuard/WireGuardDriver.cs#L17) | (suy luận) — UDP-only, static point-to-point, `SecurityKinds = Noise`, `AddressAssignment.OutOfBand`; logic ở WireGuard |
| Nebula (Noise_IX_25519_AESGCM_SHA256 + AES-256-GCM) | `NebulaDriver` qua `UseNebula()` | [VpnClientBuilder.cs:226-230](VpnClientBuilder.cs#L226-L230), [NebulaDriver.cs:17](../TqkLibrary.VpnClient.Drivers.Nebula/NebulaDriver.cs#L17) | (suy luận) — UDP, certificate auth chống network CA, data plane type-1 (Message); logic ở Nebula |
| Tailscale ts2021 (Noise IK control + WireGuard data) | `TailscaleDriver` qua `UseTailscale()` | [VpnClientBuilder.cs:241-245](VpnClientBuilder.cs#L241-L245), [TailscaleDriver.cs:19](../TqkLibrary.VpnClient.Drivers.Tailscale/TailscaleDriver.cs#L19) | (suy luận) — control plane Noise IK → Headscale/Tailscale + netmap, data plane WireGuard multi-peer; DERP/disco là future work |
| tinc 1.1 SPTPS (Curve25519/Ed25519/ChaCha-Poly1305) | `TincDriver` qua `UseTinc()` | [VpnClientBuilder.cs:254-258](VpnClientBuilder.cs#L254-L258), [TincDriver.cs:17](../TqkLibrary.VpnClient.Drivers.Tinc/TincDriver.cs#L17) | (suy luận) — TCP meta-connection + per-tunnel SPTPS + bare-IP router-mode over UDP; logic ở Tinc |
| n2n v3 (ntop: NULL / AES-CBC transform) | `N2nDriver` qua `UseN2n()` | [VpnClientBuilder.cs:268-272](VpnClientBuilder.cs#L268-L272), [N2nDriver.cs:17](../TqkLibrary.VpnClient.Drivers.N2n/N2nDriver.cs#L17) | (suy luận) — UDP tới supernode (REGISTER_SUPER), PACKET Ethernet frames, L2 bridge; logic ở N2n |
| vtun (legacy: MD5-keyed Blowfish-ECB challenge) | `VtunDriver` qua `UseVtun()` | [VpnClientBuilder.cs:283-287](VpnClientBuilder.cs#L283-L287), [VtunDriver.cs:18](../TqkLibrary.VpnClient.Drivers.Vtun/VtunDriver.cs#L18) | (suy luận) — 1 TCP, challenge-response, bare IP length-prefixed, ECHO keepalive; **crypto legacy/yếu — interop only** |
| SSH-2 transport (RFC 4251/4253) + `tun@openssh.com` | `SshDriver` qua `UseSsh()` | [VpnClientBuilder.cs:299-303](VpnClientBuilder.cs#L299-L303), [SshDriver.cs:18](../TqkLibrary.VpnClient.Drivers.Ssh/SshDriver.cs#L18) | (suy luận) — curve25519-sha256 KEX + ed25519 host-key + publickey/password, point-to-point L3 tun, bare IP; logic ở Ssh |
| ZeroTier VL1/VL2 (Curve25519 + Salsa20/12 + Poly1305) | `ZeroTierDriver` qua `UseZeroTier()` | [VpnClientBuilder.cs:316-320](VpnClientBuilder.cs#L316-L320), [ZeroTierDriver.cs:18](../TqkLibrary.VpnClient.Drivers.ZeroTier/ZeroTierDriver.cs#L18) | (suy luận) — UDP HELLO⇄OK identity agreement, NETWORK_CONFIG_REQUEST, EXT_FRAME L2 bridge; logic ở ZeroTier |
| OpenConnect (Cisco AnyConnect / ocserv CSTP) + RFC 6347 (DTLS 1.2) | `OpenConnectDriver` qua `UseOpenConnect()` | [VpnClientBuilder.cs:327-339](VpnClientBuilder.cs#L327-L339), [OpenConnectDriver.cs:20](../TqkLibrary.VpnClient.Drivers.OpenConnect/OpenConnectDriver.cs#L20) | (suy luận) — HTTPS config-auth → CSTP, bare IP (no PPP), `X-CSTP-DPD`; data plane DTLS 1.2 (V5.c) fallback TLS — DTLS ở [Transport.Dtls](../TqkLibrary.VpnClient.Transport.Dtls) |
| SoftEther SSL-VPN (Ethernet-over-TLS) | `SoftEtherDriver` qua `UseSoftEther()` | [VpnClientBuilder.cs:345-360](VpnClientBuilder.cs#L345-L360), [SoftEtherDriver.cs:23](../TqkLibrary.VpnClient.Drivers.SoftEther/SoftEtherDriver.cs#L23) | (suy luận) — L2 segment, DHCP-leased IP (`AddressAssignment.Dhcp`), SHA-0 password auth; logic ở SoftEther |
| RFC 7348 (VXLAN) | `VxlanDriver` qua `UseVxlan()` | [VpnClientBuilder.cs:362-374](VpnClientBuilder.cs#L362-L374), [VxlanDriver.cs:20](../TqkLibrary.VpnClient.Drivers.Vxlan/VxlanDriver.cs#L20) | (suy luận) — L2-over-UDP/**4789**, VXLAN header 8B (flags 0x08 + VNI 24-bit) → Ethernet L2 fabric, static unicast remote VTEP; **không mã hóa**, **không control plane**, **không elevate/raw socket**; logic ở Vxlan |
| draft-ietf-nvo3-vxlan-gpe (VXLAN-GPE) | `VxlanGpeDriver` qua `UseVxlanGpe()` | [VpnClientBuilder.cs:376-391](VpnClientBuilder.cs#L376-L391), [VxlanGpeDriver.cs:21](../TqkLibrary.VpnClient.Drivers.VxlanGpe/VxlanGpeDriver.cs#L21) | (suy luận + clean-room từ draft) — L2-over-UDP/**4790**, header VXLAN-GPE 8B (flags I\|P `0x0C` + Next-Protocol byte3 + VNI 24-bit) — **superset VXLAN** thêm bit P + Next-Protocol (0x01 IPv4/0x02 IPv6/**0x03 Ethernet**/0x04 NSH) → Ethernet L2 fabric, static unicast remote; ingress drop Ver≠0/thiếu I\|P/OAM/Next-Protocol≠Ethernet; **chỉ Ethernet đã wire** (IPv4/IPv6/NSH parse/validate); **không mã hóa**, **không control plane**, **không elevate/raw socket**; logic ở VxlanGpe |
| RFC 8926 (Geneve) | `GeneveDriver` qua `UseGeneve()` | [VpnClientBuilder.cs:393-408](VpnClientBuilder.cs#L393-L408), [GeneveDriver.cs:21](../TqkLibrary.VpnClient.Drivers.Geneve/GeneveDriver.cs#L21) | (suy luận) — L2-over-UDP/**6081**, base header 8B (Ver/OptLen + proto-type 0x6558 + VNI 24-bit) + options TLV biến thiên → Ethernet L2 fabric, static unicast remote; receiver skip options theo OptLen + drop critical-option lạ (§3.5); sibling VXLAN; **không mã hóa**, **không control plane**, **không elevate/raw socket**; logic ở Geneve |
| RFC 3931 (L2TPv3) + RFC 4719 (Ethernet PW) | `L2tpv3EthDriver` qua `UseL2tpv3Ethernet()` | [VpnClientBuilder.cs:410-427](VpnClientBuilder.cs#L410-L427), [L2tpv3EthDriver.cs:22](../TqkLibrary.VpnClient.Drivers.L2tpv3Eth/L2tpv3EthDriver.cs#L22) | (suy luận) — L2-over-UDP/**1701** static/unmanaged, header data L2TPv3 (Session ID 32-bit + Cookie 0/4/8 + Default L2-Specific Sublayer 4B tùy chọn) → Ethernet L2 fabric, static unicast remote; Session ID + Cookie tĩnh 2 đầu, **không control plane**; ingress drop control/zero-SID/mismatch/cookie/truncated; chạy header L2TPv3-over-IP thẳng trong UDP (no-admin, KHÁC Linux `encap udp` prefix 4B); sibling VXLAN/Geneve; **không mã hóa**, **không elevate/raw socket**; logic ở L2tpv3Eth |
| draft-massar-v6ops-ayiya-02 (AYIYA) + shared-secret SHA-1 sig (aiccu) + RFC 3053 (TIC — bỏ qua ở static mode) | `AyiyaDriver` qua `UseAyiya()` | [VpnClientBuilder.cs:443-448](VpnClientBuilder.cs#L443-L448), [AyiyaDriver.cs:21](../TqkLibrary.VpnClient.Drivers.Ayiya/AyiyaDriver.cs#L21) | (suy luận + clean-room từ draft) — tunnel-broker IPv6-over-UDP/**5072** **static mode** (bỏ control-channel TIC): gói IPv6 sau header AYIYA (header 4B idlen/idtype‖siglen/hshmeth‖autmeth/opcode‖nextheader + epoch time 4B BE + identity 2^idlen + signature siglen×4), **ký toàn vẹn shared-secret SHA-1** (đặt SHA1(password) vào ô signature → hash toàn gói → ghi đè) + **chống replay bằng epoch time**; tái dùng `RawIpPassthroughChannel` (IPv6) của IpEncap qua decorator ký/kiểm; ⚠️ **KHÔNG mã hóa payload**, **không control-plane động**, **không elevate/raw socket**; logic ở Drivers.Ayiya (SHA-1 dùng thẳng BCL) |
| 3GPP TS 29.281 (GTP-U) | `GtpUDriver` qua `UseGtpU()` | [VpnClientBuilder.cs:450-464](VpnClientBuilder.cs#L450-L464), [GtpUDriver.cs:21](../TqkLibrary.VpnClient.Drivers.GtpU/GtpUDriver.cs#L21) | (suy luận + clean-room từ TS 29.281) — tunnel user-plane inner-IP-over-UDP/**2152** **static mode** (không GTP-C): gói IP sau header G-PDU 8B (Version 1 \| PT \| E/S/PN ‖ message type 255 ‖ Length ‖ TEID 32-bit BE) gắn `Teid`, optional Sequence Number, extension header skip theo length; tái dùng `RawIpPassthroughChannel` của IpEncap qua decorator framing; ingress drop non-version-1 / non-G-PDU (Echo/Error-Indication) / TEID-lệch / runt / length-sai; ⚠️ **KHÔNG mã hóa/xác thực** (chỉ gắn TEID), **không control plane**, **không elevate/raw socket**; logic ở Drivers.GtpU |
| RFC 8086 (GRE-in-UDP) + RFC 2784/2890 (GRE) + RFC 7637 (NVGRE) | `EoGreDriver` qua `UseEoGre()` / `UseNvgre()` | [VpnClientBuilder.cs:478-495](VpnClientBuilder.cs#L478-L495), [EoGreDriver.cs:24](../TqkLibrary.VpnClient.Drivers.EoGre/EoGreDriver.cs#L24) | (suy luận + clean-room từ RFC) — L2-over-UDP/**4754**: Ethernet frame sau header GRE (proto-type 0x6558 TEB) trong payload UDP → Ethernet L2 fabric; `eogre` = GRETAP (VSID tùy chọn trong RFC 2890 Key + Checksum/Sequence tùy chọn), `nvgre` = RFC 7637 (VSID 24-bit + FlowID 8-bit bắt buộc trong Key, không Checksum/Sequence, drop VSID-mismatch); **tái dùng `GreCodec`/`GrePacket` của IpEncap** (không viết lại GRE); sibling L2 của GRE-in-UDP L3; ⚠️ **KHÔNG mã hóa** (chỉ tunnel L2), **không control plane**, **không elevate/raw socket**; logic ở Drivers.EoGre |
...
- `VpnClientBuilder` →
  ...
  - `UseGtpU(GtpUOptions?, GtpUReconnectOptions?)`
  - `UseEoGre(EoGreConfig)`, `UseEoGre(EoGreConfig, EoGreReconnectOptions)`
  - `UseNvgre(EoGreConfig)`, `UseNvgre(EoGreConfig, EoGreReconnectOptions)`
...
1. **Đăng ký (build-time).** Các shortcut `UseSstp()`/`UseL2tpIpsec()`/`UsePptp()`/`UseIpEncap()`/`UseGreInUdp()`/`UseFou()`/`UseGue()`/`UseIkev2()`/`UseCiscoIpsec()`/`UseOpenVpn()`/`UseWireGuard()`/`UseNebula()`/`UseTailscale()`/`UseTinc()`/`UseN2n()`/`UseVtun()`/`UseSsh()`/`UseZeroTier()`/`UseOpenConnect()`/`UseSoftEther()`/`UseVxlan()`/`UseVxlanGpe()`/`UseGeneve()`/`UseL2tpv3Ethernet()`/`UseAyiya()`/`UseGtpU()`/`UseEoGre()`/`UseNvgre()` tạo driver cụ thể và gọi `AddDriver` để nạp vào `Dictionary<string, IVpnProtocolDriver>` keyed theo `driver.Name` (`"sstp"`, `"l2tp-ipsec"`, `"pptp"`, `"ipencap"`, `"gre-udp"`, `"fou"`, `"gue"`, `"ikev2"`, `"cisco-ipsec"`, `"openvpn"`, `"wireguard"`, `"nebula"`, `"tailscale"`, `"tinc"`, `"n2n"`, `"vtun"`, `"ssh"`, `"zerotier"`, `"openconnect"`, `"softether"`, `"vxlan"`, `"vxlan-gpe"`, `"geneve"`, `"l2tpv3-eth"`, `"ayiya"`, `"gtpu"`) — [VpnClientBuilder.cs:57-499](VpnClientBuilder.cs#L57-L499). `Build()` đóng gói dictionary đó vào `VpnClient` — [VpnClientBuilder.cs:499](VpnClientBuilder.cs#L499).
...
Toàn bộ việc lắp ráp stack (IKE → ESP → L2TP → PPP → IP cho L2TP; TLS/CSTP/DTLS, GRE, VXLAN, Noise, SSH, Ethernet-over-TLS cho các giao thức khác) diễn ra **bên trong driver**, không nằm ở project này. 28 driver nằm ở 26 project anh em (Drivers.Fou cung cấp cả `fou` lẫn `gue`): [Sstp](../TqkLibrary.VpnClient.Drivers.Sstp), [L2tpIpsec](../TqkLibrary.VpnClient.Drivers.L2tpIpsec), [Pptp](../TqkLibrary.VpnClient.Drivers.Pptp), [IpEncap](../TqkLibrary.VpnClient.Drivers.IpEncap), [GreInUdp](../TqkLibrary.VpnClient.Drivers.GreInUdp), [Fou](../TqkLibrary.VpnClient.Drivers.Fou), [GtpU](../TqkLibrary.VpnClient.Drivers.GtpU) (GTP-U static-mode inner-IP-over-UDP/2152), [Vxlan](../TqkLibrary.VpnClient.Drivers.Vxlan), [VxlanGpe](../TqkLibrary.VpnClient.Drivers.VxlanGpe) (VXLAN-GPE superset VXLAN + Next-Protocol), [Geneve](../TqkLibrary.VpnClient.Drivers.Geneve), [L2tpv3Eth](../TqkLibrary.VpnClient.Drivers.L2tpv3Eth), [Ayiya](../TqkLibrary.VpnClient.Drivers.Ayiya) (AYIYA static-mode IPv6-over-UDP), [Ikev2](../TqkLibrary.VpnClient.Drivers.Ikev2) (IKEv2 + ESP tunnel mode, không PPP), [CiscoIpsec](../TqkLibrary.VpnClient.Drivers.CiscoIpsec), [OpenVpn](../TqkLibrary.VpnClient.Drivers.OpenVpn), [WireGuard](../TqkLibrary.VpnClient.Drivers.WireGuard), [Nebula](../TqkLibrary.VpnClient.Drivers.Nebula), [Tailscale](../TqkLibrary.VpnClient.Drivers.Tailscale), [Tinc](../TqkLibrary.VpnClient.Drivers.Tinc), [N2n](../TqkLibrary.VpnClient.Drivers.N2n), [Vtun](../TqkLibrary.VpnClient.Drivers.Vtun), [Ssh](../TqkLibrary.VpnClient.Drivers.Ssh), [ZeroTier](../TqkLibrary.VpnClient.Drivers.ZeroTier), [OpenConnect](../TqkLibrary.VpnClient.Drivers.OpenConnect), [EoGre](../TqkLibrary.VpnClient.Drivers.EoGre) (EoGRE/NVGRE static-mode Ethernet-over-GRE-in-UDP/4754) và [SoftEther](../TqkLibrary.VpnClient.Drivers.SoftEther). Ví dụ điều phối L2TP/IPsec: `L2tpIpsecDriver.ConnectAsync` kiểm PSK bắt buộc rồi dựng `L2tpIpsecConnection` và gọi `ConnectAsync` của nó — [L2tpIpsecDriver.cs:58-88](../TqkLibrary.VpnClient.Drivers.L2tpIpsec/L2tpIpsecDriver.cs#L58-L88). Chi tiết luồng handshake xem [.docs/10 §6](../../.docs/10-codebase-architecture-and-flow.md).

## Trạng thái & ghi chú

- **Driver đã wire:** `sstp`, `l2tp-ipsec`, `pptp`, `ipencap`, `gre-udp`, `fou`, `gue`, `ikev2`, `cisco-ipsec`, `openvpn`, `wireguard`, `nebula`, `tailscale`, `tinc`, `n2n`, `vtun`, `ssh`, `zerotier`, `openconnect`, `softether`, `vxlan`, `vxlan-gpe`, `geneve`, `l2tpv3-eth`, `ayiya`, `gtpu`, `eogre`, `nvgre` (qua 28 nhóm shortcut `UseSstp()`/`UseL2tpIpsec()`/`UsePptp()`/`UseIpEncap()`/`UseGreInUdp()`/`UseFou()`/`UseGue()`/`UseIkev2()`/`UseCiscoIpsec()`/`UseOpenVpn()`/`UseWireGuard()`/`UseNebula()`/`UseTailscale()`/`UseTinc()`/`UseN2n()`/`UseVtun()`/`UseSsh()`/`UseZeroTier()`/`UseOpenConnect()`/`UseSoftEther()`/`UseVxlan()`/`UseVxlanGpe()`/`UseGeneve()`/`UseL2tpv3Ethernet()`/`UseAyiya()`/`UseGtpU()`/`UseEoGre()`/`UseNvgre()`). Driver tùy ý khác có thể nạp qua `AddDriver(IVpnProtocolDriver)`.
