...
- Cho ứng dụng một API gọn để **đăng ký các protocol driver** (SSTP, L2TP/IPsec, PPTP, IP-encap GRE/IPIP/SIT, GRE-in-UDP, FOU/GUE, VXLAN, Geneve, L2TPv3-eth, AYIYA, IKEv2, Cisco IPsec/EzVPN, OpenVPN, WireGuard, Nebula, Tailscale, tinc, n2n, vtun, VPN-over-SSH, ZeroTier, OpenConnect, SoftEther) theo kiểu fluent rồi **mở kết nối theo tên giao thức**.
...
- Là nơi **đảo ngược phụ thuộc** hội tụ: façade biết về các driver cụ thể (`SstpDriver`, `L2tpIpsecDriver`, `PptpDriver`, `IpEncapDriver`, `GreInUdpDriver`, `FouDriver`, `VxlanDriver`, `GeneveDriver`, `L2tpv3EthDriver`, `AyiyaDriver`, `Ikev2Driver`, `CiscoIpsecDriver`, `OpenVpnDriver`, `WireGuardDriver`, `NebulaDriver`, `TailscaleDriver`, `TincDriver`, `N2nDriver`, `VtunDriver`, `SshDriver`, `ZeroTierDriver`, `OpenConnectDriver`, `SoftEtherDriver`), còn mọi tầng dưới chỉ phụ thuộc vào abstractions.
...
- **Phụ thuộc (ProjectReference) — 23 project driver (không ref `Sockets`):**
...
```
TqkLibrary.VpnClient/
├── VpnClientBuilder.cs   # Builder fluent: AddDriver / UseSstp / UseL2tpIpsec / UsePptp / UseIpEncap / UseGreInUdp / UseFou / UseGue / UseIkev2 / UseCiscoIpsec / UseOpenVpn / UseWireGuard / UseNebula / UseTailscale / UseTinc / UseN2n / UseVtun / UseSsh / UseZeroTier / UseOpenConnect / UseSoftEther / UseVxlan / UseGeneve / UseL2tpv3Ethernet / UseAyiya → Build()
...
Bản thân project façade **không hiện thực chuẩn mạng nào** — nó chỉ điều phối driver. Các chuẩn dưới đây được **truy cập gián tiếp** qua các shortcut `UseSstp()`/`UseL2tpIpsec()`/`UsePptp()`/`UseIpEncap()`/`UseGreInUdp()`/`UseFou()`/`UseGue()`/`UseIkev2()`/`UseCiscoIpsec()`/`UseOpenVpn()`/`UseWireGuard()`/`UseNebula()`/`UseTailscale()`/`UseTinc()`/`UseN2n()`/`UseVtun()`/`UseSsh()`/`UseZeroTier()`/`UseOpenConnect()`/`UseSoftEther()`/`UseVxlan()`/`UseGeneve()`/`UseL2tpv3Ethernet()`/`UseAyiya()`; cột "Vị trí" link tới nơi façade kích hoạt driver tương ứng, và (nếu có) tới class summary ở driver. Project này không có comment `RFC` nào, nên gần như toàn bộ là **(suy luận)** — chi tiết ánh xạ chuẩn → code nằm ở README của các project Drivers/Ipsec/L2tp/Ppp/OpenVpn/WireGuard/SoftEther/Transport.Dtls.

| Chuẩn (RFC/FIPS/NIST/MS-*) | Class/Namespace áp dụng | Vị trí (link code) | Ghi chú |
| --- | --- | --- | --- |
| [MS-SSTP] (Secure Socket Tunneling Protocol) | `SstpDriver` qua `UseSstp()` | [VpnClientBuilder.cs:58-69](VpnClientBuilder.cs#L58-L69), [SstpDriver.cs:11](../TqkLibrary.VpnClient.Drivers.Sstp/SstpDriver.cs#L11) | Driver mô tả "TLS over 443, PPP, MS-CHAPv2"; cert TLS validate qua callback tùy chọn (P0.6) |
| RFC 2759 (MS-CHAPv2) | `SstpDriver` + `L2tpIpsecDriver` + `PptpDriver` (PPP auth) | [VpnClientBuilder.cs:58](VpnClientBuilder.cs#L58), [VpnClientBuilder.cs:72](VpnClientBuilder.cs#L72), [VpnClientBuilder.cs:100-102](VpnClientBuilder.cs#L100-L102) | (suy luận) — codec ở Crypto/`MsChapV2`, framing CHAP ở Ppp/`MsChapV2Authenticator` |
| RFC 2409 (IKEv1 / ISAKMP) | `L2tpIpsecDriver` qua `UseL2tpIpsec()` + `CiscoIpsecDriver` qua `UseCiscoIpsec()` (Aggressive Mode) | [VpnClientBuilder.cs:72-89](VpnClientBuilder.cs#L72-L89), [L2tpIpsecDriver.cs:12](../TqkLibrary.VpnClient.Drivers.L2tpIpsec/L2tpIpsecDriver.cs#L12), [CiscoIpsecDriver.cs:18](../TqkLibrary.VpnClient.Drivers.CiscoIpsec/CiscoIpsecDriver.cs#L18) | (suy luận) — comment driver "IKEv1 PSK over NAT-T"; logic ở Ipsec `Ike/V1` |
| RFC 7296 (IKEv2) + ESP tunnel mode + CP | `Ikev2Driver` qua `UseIkev2()` / `UseIkev2(IkeCertificateTrust)` | [VpnClientBuilder.cs:164-177](VpnClientBuilder.cs#L164-L177), [Ikev2Driver.cs:11](../TqkLibrary.VpnClient.Drivers.Ikev2/Ikev2Driver.cs#L11) | (suy luận) — driver "RFC 7296 PSK over NAT-T, CP virtual IP, ESP tunnel mode — no PPP"; logic ở Ipsec `Ike/V2` + `Esp` |
...
| RFC 2637 (PPTP) + RFC 3078/3079 (MPPE) | `PptpDriver` qua `UsePptp()` | [VpnClientBuilder.cs:100-102](VpnClientBuilder.cs#L100-L102), [PptpDriver.cs:16](../TqkLibrary.VpnClient.Drivers.Pptp/PptpDriver.cs#L16) | (suy luận) — TCP/1723 control + GRE proto-47 data + MPPE/RC4 + PPP/MS-CHAPv2; **legacy/không an toàn — interop only** |
| RFC 2784/2890 (GRE), RFC 2003 (IPIP), RFC 4213 (SIT/6in4) | `IpEncapDriver` qua `UseIpEncap()` (+ data plane GRE của `PptpDriver`) | [VpnClientBuilder.cs:114-116](VpnClientBuilder.cs#L114-L116), [IpEncapDriver.cs:20](../TqkLibrary.VpnClient.Drivers.IpEncap/IpEncapDriver.cs#L20) | (suy luận) — tunnel IP-in-IP **không mã hóa**, không control plane; raw IP proto-47/4/41 |
| RFC 8086 (GRE-in-UDP) + RFC 2784/2890 (GRE) | `GreInUdpDriver` qua `UseGreInUdp()` | [VpnClientBuilder.cs:119-130](VpnClientBuilder.cs#L119-L130), [GreInUdpDriver.cs:19](../TqkLibrary.VpnClient.Drivers.GreInUdp/GreInUdpDriver.cs#L19) | (suy luận) — header GRE trong payload **UDP/4754** thay raw-IP proto-47 — **không mã hóa**, **không elevate/raw socket**, qua NAT; tái dùng `GreCodec` của IpEncap |
| draft-ietf-intarea-gue (GUE) + FOU (Linux Foo-over-UDP) + RFC 2784/2890 (GRE) / RFC 2003 (IPIP) / RFC 4213 (SIT) | `FouDriver` qua `UseFou()` / `UseGue()` | [VpnClientBuilder.cs:132-161](VpnClientBuilder.cs#L132-L161), [FouDriver.cs:21](../TqkLibrary.VpnClient.Drivers.Fou/FouDriver.cs#L21) | (suy luận) — inner IP-proto (IPIP-4/41 hoặc GRE-47) trong payload **UDP**: FOU không header (proto suy từ port), GUE thêm header variant-0 4B mang Proto (skip ext theo Hlen, drop C-bit); tổng quát hoá GRE-in-UDP; **không mã hóa**, **không elevate/raw socket**; tái dùng `GreCodec`/`RawIpPassthroughChannel` của IpEncap |
...
| OpenVPN (community-server protocol) + NCP AEAD | `OpenVpnDriver` qua `UseOpenVpn()` | [VpnClientBuilder.cs:192-205](VpnClientBuilder.cs#L192-L205), [OpenVpnDriver.cs:24](../TqkLibrary.VpnClient.Drivers.OpenVpn/OpenVpnDriver.cs#L24) | (suy luận) — UDP/TCP, tun-mode (L3) / tap-mode (L2), tls-auth/tls-crypt; logic ở OpenVpn |
| WireGuard (Noise_IKpsk2 + ChaCha20-Poly1305) | `WireGuardDriver` qua `UseWireGuard()` (+ data plane của `TailscaleDriver`) | [VpnClientBuilder.cs:208-212](VpnClientBuilder.cs#L208-L212), [WireGuardDriver.cs:17](../TqkLibrary.VpnClient.Drivers.WireGuard/WireGuardDriver.cs#L17) | (suy luận) — UDP-only, static point-to-point, `SecurityKinds = Noise`, `AddressAssignment.OutOfBand`; logic ở WireGuard |
| Nebula (Noise_IX_25519_AESGCM_SHA256 + AES-256-GCM) | `NebulaDriver` qua `UseNebula()` | [VpnClientBuilder.cs:220-224](VpnClientBuilder.cs#L220-L224), [NebulaDriver.cs:17](../TqkLibrary.VpnClient.Drivers.Nebula/NebulaDriver.cs#L17) | (suy luận) — UDP, certificate auth chống network CA, data plane type-1 (Message); logic ở Nebula |
| Tailscale ts2021 (Noise IK control + WireGuard data) | `TailscaleDriver` qua `UseTailscale()` | [VpnClientBuilder.cs:235-239](VpnClientBuilder.cs#L235-L239), [TailscaleDriver.cs:19](../TqkLibrary.VpnClient.Drivers.Tailscale/TailscaleDriver.cs#L19) | (suy luận) — control plane Noise IK → Headscale/Tailscale + netmap, data plane WireGuard multi-peer; DERP/disco là future work |
| tinc 1.1 SPTPS (Curve25519/Ed25519/ChaCha-Poly1305) | `TincDriver` qua `UseTinc()` | [VpnClientBuilder.cs:248-252](VpnClientBuilder.cs#L248-L252), [TincDriver.cs:17](../TqkLibrary.VpnClient.Drivers.Tinc/TincDriver.cs#L17) | (suy luận) — TCP meta-connection + per-tunnel SPTPS + bare-IP router-mode over UDP; logic ở Tinc |
| n2n v3 (ntop: NULL / AES-CBC transform) | `N2nDriver` qua `UseN2n()` | [VpnClientBuilder.cs:262-266](VpnClientBuilder.cs#L262-L266), [N2nDriver.cs:17](../TqkLibrary.VpnClient.Drivers.N2n/N2nDriver.cs#L17) | (suy luận) — UDP tới supernode (REGISTER_SUPER), PACKET Ethernet frames, L2 bridge; logic ở N2n |
| vtun (legacy: MD5-keyed Blowfish-ECB challenge) | `VtunDriver` qua `UseVtun()` | [VpnClientBuilder.cs:277-281](VpnClientBuilder.cs#L277-L281), [VtunDriver.cs:18](../TqkLibrary.VpnClient.Drivers.Vtun/VtunDriver.cs#L18) | (suy luận) — 1 TCP, challenge-response, bare IP length-prefixed, ECHO keepalive; **crypto legacy/yếu — interop only** |
| SSH-2 transport (RFC 4251/4253) + `tun@openssh.com` | `SshDriver` qua `UseSsh()` | [VpnClientBuilder.cs:293-297](VpnClientBuilder.cs#L293-L297), [SshDriver.cs:18](../TqkLibrary.VpnClient.Drivers.Ssh/SshDriver.cs#L18) | (suy luận) — curve25519-sha256 KEX + ed25519 host-key + publickey/password, point-to-point L3 tun, bare IP; logic ở Ssh |
| ZeroTier VL1/VL2 (Curve25519 + Salsa20/12 + Poly1305) | `ZeroTierDriver` qua `UseZeroTier()` | [VpnClientBuilder.cs:310-314](VpnClientBuilder.cs#L310-L314), [ZeroTierDriver.cs:18](../TqkLibrary.VpnClient.Drivers.ZeroTier/ZeroTierDriver.cs#L18) | (suy luận) — UDP HELLO⇄OK identity agreement, NETWORK_CONFIG_REQUEST, EXT_FRAME L2 bridge; logic ở ZeroTier |
| OpenConnect (Cisco AnyConnect / ocserv CSTP) + RFC 6347 (DTLS 1.2) | `OpenConnectDriver` qua `UseOpenConnect()` | [VpnClientBuilder.cs:321-333](VpnClientBuilder.cs#L321-L333), [OpenConnectDriver.cs:20](../TqkLibrary.VpnClient.Drivers.OpenConnect/OpenConnectDriver.cs#L20) | (suy luận) — HTTPS config-auth → CSTP, bare IP (no PPP), `X-CSTP-DPD`; data plane DTLS 1.2 (V5.c) fallback TLS — DTLS ở [Transport.Dtls](../TqkLibrary.VpnClient.Transport.Dtls) |
| SoftEther SSL-VPN (Ethernet-over-TLS) | `SoftEtherDriver` qua `UseSoftEther()` | [VpnClientBuilder.cs:339-354](VpnClientBuilder.cs#L339-L354), [SoftEtherDriver.cs:23](../TqkLibrary.VpnClient.Drivers.SoftEther/SoftEtherDriver.cs#L23) | (suy luận) — L2 segment, DHCP-leased IP (`AddressAssignment.Dhcp`), SHA-0 password auth; logic ở SoftEther |
| RFC 7348 (VXLAN) | `VxlanDriver` qua `UseVxlan()` | [VpnClientBuilder.cs:356-368](VpnClientBuilder.cs#L356-L368), [VxlanDriver.cs:20](../TqkLibrary.VpnClient.Drivers.Vxlan/VxlanDriver.cs#L20) | (suy luận) — L2-over-UDP/**4789**, VXLAN header 8B (flags 0x08 + VNI 24-bit) → Ethernet L2 fabric, static unicast remote VTEP; **không mã hóa**, **không control plane**, **không elevate/raw socket**; logic ở Vxlan |
| RFC 8926 (Geneve) | `GeneveDriver` qua `UseGeneve()` | [VpnClientBuilder.cs:370-385](VpnClientBuilder.cs#L370-L385), [GeneveDriver.cs:21](../TqkLibrary.VpnClient.Drivers.Geneve/GeneveDriver.cs#L21) | (suy luận) — L2-over-UDP/**6081**, base header 8B (Ver/OptLen + proto-type 0x6558 + VNI 24-bit) + options TLV biến thiên → Ethernet L2 fabric, static unicast remote; receiver skip options theo OptLen + drop critical-option lạ (§3.5); sibling VXLAN; **không mã hóa**, **không control plane**, **không elevate/raw socket**; logic ở Geneve |
| RFC 3931 (L2TPv3) + RFC 4719 (Ethernet PW) | `L2tpv3EthDriver` qua `UseL2tpv3Ethernet()` | [VpnClientBuilder.cs:387-404](VpnClientBuilder.cs#L387-L404), [L2tpv3EthDriver.cs:22](../TqkLibrary.VpnClient.Drivers.L2tpv3Eth/L2tpv3EthDriver.cs#L22) | (suy luận) — L2-over-UDP/**1701** static/unmanaged, header data L2TPv3 (Session ID 32-bit + Cookie 0/4/8 + Default L2-Specific Sublayer 4B tùy chọn) → Ethernet L2 fabric, static unicast remote; Session ID + Cookie tĩnh 2 đầu, **không control plane**; ingress drop control/zero-SID/mismatch/cookie/truncated; chạy header L2TPv3-over-IP thẳng trong UDP (no-admin, KHÁC Linux `encap udp` prefix 4B); sibling VXLAN/Geneve; **không mã hóa**, **không elevate/raw socket**; logic ở L2tpv3Eth |
| draft-massar-v6ops-ayiya-02 (AYIYA) + shared-secret SHA-1 sig (aiccu) + RFC 3053 (TIC — bỏ qua ở static mode) | `AyiyaDriver` qua `UseAyiya()` | [VpnClientBuilder.cs:420-425](VpnClientBuilder.cs#L420-L425), [AyiyaDriver.cs:21](../TqkLibrary.VpnClient.Drivers.Ayiya/AyiyaDriver.cs#L21) | (suy luận + clean-room từ draft) — tunnel-broker IPv6-over-UDP/**5072** **static mode** (bỏ control-channel TIC): gói IPv6 sau header AYIYA (header 4B idlen/idtype‖siglen/hshmeth‖autmeth/opcode‖nextheader + epoch time 4B BE + identity 2^idlen + signature siglen×4), **ký toàn vẹn shared-secret SHA-1** (đặt SHA1(password) vào ô signature → hash toàn gói → ghi đè) + **chống replay bằng epoch time**; tái dùng `RawIpPassthroughChannel` (IPv6) của IpEncap qua decorator ký/kiểm; ⚠️ **KHÔNG mã hóa payload**, **không control-plane động**, **không elevate/raw socket**; logic ở Drivers.Ayiya (SHA-1 dùng thẳng BCL) |
...
1. **Đăng ký (build-time).** Các shortcut `UseSstp()`/`UseL2tpIpsec()`/`UsePptp()`/`UseIpEncap()`/`UseGreInUdp()`/`UseFou()`/`UseGue()`/`UseIkev2()`/`UseCiscoIpsec()`/`UseOpenVpn()`/`UseWireGuard()`/`UseNebula()`/`UseTailscale()`/`UseTinc()`/`UseN2n()`/`UseVtun()`/`UseSsh()`/`UseZeroTier()`/`UseOpenConnect()`/`UseSoftEther()`/`UseVxlan()`/`UseGeneve()`/`UseL2tpv3Ethernet()`/`UseAyiya()` tạo driver cụ thể và gọi `AddDriver` để nạp vào `Dictionary<string, IVpnProtocolDriver>` keyed theo `driver.Name` (`"sstp"`, `"l2tp-ipsec"`, `"pptp"`, `"ipencap"`, `"gre-udp"`, `"fou"`, `"gue"`, `"ikev2"`, `"cisco-ipsec"`, `"openvpn"`, `"wireguard"`, `"nebula"`, `"tailscale"`, `"tinc"`, `"n2n"`, `"vtun"`, `"ssh"`, `"zerotier"`, `"openconnect"`, `"softether"`, `"vxlan"`, `"geneve"`, `"l2tpv3-eth"`, `"ayiya"`) — [VpnClientBuilder.cs:51-428](VpnClientBuilder.cs#L51-L428). `Build()` đóng gói dictionary đó vào `VpnClient` — [VpnClientBuilder.cs:428](VpnClientBuilder.cs#L428).
