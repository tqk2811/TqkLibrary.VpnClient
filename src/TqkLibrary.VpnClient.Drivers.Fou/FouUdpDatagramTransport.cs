...
{
    /// <summary>
    /// A connected UDP datagram pipe over a real socket, carrying a FOU/GUE tunnel's inner payload. <b>Passive</b> by
    /// design: it exposes one-datagram <see cref="SendAsync"/> / <see cref="ReceiveAsync"/> but does <i>not</i> run its own
    /// receive pump — the <see cref="Abstractions.Channels.Interfaces.IPacketChannel"/> on top (a reused
    /// <c>GreTunnelChannel</c> / <c>RawIpPassthroughChannel</c>, optionally behind a <see cref="GueFramingTransport"/>)
    /// drives its own receive loop by calling <see cref="ReceiveAsync"/>. Supports both IPv4 and IPv6 by following the
    /// remote endpoint's <see cref="AddressFamily"/>. Mirrors the GRE-in-UDP driver's <c>UdpDatagramTransport</c>.
    /// </summary>
    internal sealed class FouUdpDatagramTransport : IDatagramTransport
    {
        ...
        public FouUdpDatagramTransport(IPEndPoint remote, IPAddress? localBind = null)
        {
        ...
        static ArraySegment<byte> GetArraySegment(Memory<byte> memory)
        {
            ...
            // (GreTunnelChannel / RawIpPassthroughChannel / GueFramingTransport) always pass an array-backed buffer.
