...
> Managed crypto: primitive contracts (Interfaces/), MD4/DES/SHA-0/SHA-256/DH-MODP/ECDH-P256 (RFC 5903 group 19)/AES-CBC/CTR/RC4/Salsa20/HMAC/RNG, AEAD (AES-GCM + ChaCha20-Poly1305: native trên net8.0, BouncyCastle trên netstandard2.0; cộng XChaCha20-Poly1305 = HChaCha20 + ChaCha20-Poly1305 cho WireGuard cookie-reply), PRF TLS 1.0 (`Tls1Prf`) + cửa sổ chống replay (`AntiReplayWindow`), MPPE/PPTP (RC4 + key schedule RFC 3078/3079), Salsa20 (rounds tham số — Salsa20/20 chuẩn + Salsa20/12 ZeroTier, KAT ECRYPT), HSalsa20/XSalsa20 + XSalsa20-Poly1305 (NaCl `crypto_secretbox`) + `NaclBox` (NaCl `crypto_box` = Curve25519 + XSalsa20-Poly1305, KAT libsodium + box.c — hoàn tất bộ NaCl, mở khóa họ mesh NaCl nacltai/cjdns), BLAKE2b + BLAKE2b-keyed (RFC 7693, output 1..64B — trám gap Yggdrasil), UMAC (RFC 4418 — MAC Wegman-Carter tag 32/64/96/128-bit, KAT §9 byte-exact — trám gap fastd), và Noise primitives (X25519/Ed25519/BLAKE2s/HMAC-BLAKE2s/Noise-KDF/SymmetricState qua BouncyCastle trên cả 2 TFM — nền cho WireGuard `Noise_IKpsk2` + Nebula `Noise_IX`).
...
│   ├── XSalsa20Poly1305Cipher.cs # XSalsa20-Poly1305 = NaCl crypto_secretbox (XSalsa20 keystream + Poly1305 one-time): họ mesh NaCl nacltai/cjdns
│   └── NaclBox.cs           # NaCl crypto_box = X25519 beforenm (HSalsa20(DH,0^16)) + secretbox (static): hoàn tất bộ NaCl — Quicktun nacltai/cjdns
...
├── HSalsa20.cs            # HSalsa20 core (DJB "Extending the Salsa20 nonce"): key32+in16→out32, 20 round no-FF — stage-1 của XSalsa20 (static)
├── XSalsa20.cs            # XSalsa20 (nonce 24B): subkey=HSalsa20(key,nonce[0:16]) → Salsa20/20(subkey,nonce[16:24]); tái dùng Salsa20 (static)
...
├── Blake2b.cs             # BLAKE2b unkeyed (RFC 7693, output 1..64B mặc định 64) — wrapper BC Blake2bDigest; Yggdrasil node-ID/tree (static)
├── Blake2bKeyedMac.cs     # keyed BLAKE2b output 1..64B / key ≤64B (RFC 7693 §2.9) — wrapper BC Blake2bDigest; MAC mesh BLAKE2b (static)
├── Umac.cs                # UMAC (RFC 4418) MAC Wegman-Carter: UHASH⊕PDF, tag 32/64/96/128B, key AES-128/256 — nền fastd (salsa2012+umac); KAT §9 (static)
...
├── NistP256DhGroup.cs     # ECDH P-256 (secp256r1/prime256v1, IANA group 19, RFC 5903) — IDhGroup (wire x‖y 64B / shared x 32B)
...
| `HSalsa20` | HSalsa20 core (DJB "Extending the Salsa20 nonce"), `static Hash(key32, in16, out32)`: 20-round Salsa20 core **không feed-forward**, output words (x0,x5,x10,x15,x6,x7,x8,x9); stage-1 của `XSalsa20`; KAT byte-exact libsodium core2/core3 | [HSalsa20.cs:18](HSalsa20.cs#L18) |
| `XSalsa20` | XSalsa20 stream cipher (nonce **24B**), `static Transform`/`GenerateKeystream`: subkey=`HSalsa20`(key,nonce[0:16]) → **tái dùng `Salsa20`/20**(subkey,nonce[16:24]); nền NaCl `crypto_secretbox`; KAT keystream libsodium | [XSalsa20.cs:15](XSalsa20.cs#L15) |
| `XSalsa20Poly1305Cipher` | XSalsa20-Poly1305 = NaCl `crypto_secretbox` (**không** AEAD/AAD), key 32B / nonce 24B / tag 16B; `static Seal(key,nonce,plain)→tag‖cipher` / `Open(key,nonce,tag,cipher,out plain)`: XSalsa20 keystream (32B đầu = khóa Poly1305 one-time, cipher = plain XOR keystream từ byte 32), tag = BouncyCastle `Poly1305` (r-half clamp RFC 8439), verify hằng-thời-gian `CryptoBytes.FixedTimeEquals`; mở khóa họ mesh NaCl nacltai/cjdns; KAT byte-exact libsodium secretbox.exp | [Aead/XSalsa20Poly1305Cipher.cs:24](Aead/XSalsa20Poly1305Cipher.cs#L24) |
| `NaclBox` | NaCl `crypto_box` (Curve25519 + XSalsa20-Poly1305), pk/sk 32B / nonce 24B / tag 16B; `static BeforeNm(theirPk,mySk)` = `HSalsa20(X25519(mySk,theirPk),0^16)` (32B shared) + `Box(plain,nonce,theirPk,mySk)→tag‖cipher` / `Open(boxed,nonce,theirPk,mySk,out plain)` + biến thể `*AfterNm` (khóa beforenm precompute); ủy quyền `Curve25519DhGroup` (X25519) + `HSalsa20` + `XSalsa20Poly1305Cipher` (**không** viết lại core); hoàn tất bộ NaCl (Quicktun nacltai/cjdns); KAT byte-exact NaCl `box.c` (Alice-sk+Bob-pk→beforenm = secretbox firstkey) | [Aead/NaclBox.cs:21](Aead/NaclBox.cs#L21) |
| `Blake2b` | BLAKE2b unkeyed (RFC 7693), output **1..64B** (mặc định 64 = BLAKE2b-512); `static Hash(input,outputBytes=64)→byte[]` / `Hash(input,Span output)`; wrapper BouncyCastle `Blake2bDigest(int bits)` (**KHÔNG** tự cài compression); Yggdrasil node-ID/tree hash; KAT RFC 7693 App.A (`"abc"`) + empty 256/512 | [Blake2b.cs:14](Blake2b.cs#L14) |
| `Blake2bKeyedMac` | Keyed BLAKE2b (RFC 7693 §2.9), key **≤64B** / output **1..64B**, `static ComputeMac(key,input,output)`; wrapper `Blake2bDigest(key,outLenBytes,salt,personal)` (outLen **BYTES**); MAC mesh BLAKE2b; KAT `blake2b-kat.txt` (key 00..3f 64B) | [Blake2bKeyedMac.cs:12](Blake2bKeyedMac.cs#L12) |
| `Umac` | UMAC (RFC 4418) MAC Wegman-Carter `UHASH ⊕ PDF`, `static ComputeTag(key,message,nonce,tagBits=64)`; **tag 32/64/96/128-bit**, key **16B (AES-128)/32B (AES-256)**, nonce 1..16B; UHASH 3 tầng = NH (đọc message **little-endian** 32-bit) + POLY mod prime(64)=2^64-59 + inner-product mod prime(36)=2^36-5; KDF/PDF bằng AES-CTR; **single-step POLY** (RFC "ramp p128" là sai lệch spec↔vector — khớp reference `umac.c`); POLY/inner-product qua `BigInteger`; nền fastd (salsa2012+umac); KAT RFC 4418 §9 byte-exact | [Umac.cs:47](Umac.cs#L47) |
| `ModpDhGroup` | DH MODP group 2 (1024-bit) / 14 (2048-bit), g=2 (`IDhGroup`) | [ModpDhGroup.cs:12](ModpDhGroup.cs#L12) |
| `NistP256DhGroup` | ECDH **P-256** (secp256r1/prime256v1, IANA **group 19**, RFC 5903), `IDhGroup`; private 32B scalar [1,n-1] / public **x‖y 64B** (KHÔNG byte point-format 0x04, §7) / shared = **x của g^ir 32B** (§9); wrapper BouncyCastle `CustomNamedCurves("P-256")` + `ECPoint.Multiply` cả 2 TFM (không `#if` — BCL không expose raw P-256 scalar-mult/agreement), điểm peer validate on-curve qua `DecodePoint`/`IsValid` (cofactor 1); KAT RFC 5903 §8.1 byte-exact — mở khóa Nebula-P256/FreeLAN ECDHE | [NistP256DhGroup.cs:22](NistP256DhGroup.cs#L22) |
...
| RFC 5903 (ECP Groups cho IKE/IKEv2 — P-256 group 19) | `NistP256DhGroup` | [NistP256DhGroup.cs:16](NistP256DhGroup.cs#L16) | Comment; wire x‖y 64B (không byte 0x04, §7) + shared x-coord 32B (§9); KAT vector §8.1 byte-exact (public g^i/g^r + shared g^ir) |
...
| DJB "Extending the Salsa20 nonce" (HSalsa20 / XSalsa20) | `HSalsa20`, `XSalsa20` | [HSalsa20.cs:6](HSalsa20.cs#L6), [XSalsa20.cs:6](XSalsa20.cs#L6) | Comment; KAT HSalsa20 libsodium core2/core3 + XSalsa20 keystream (`XSalsa20Poly1305Tests`) |
| NaCl `crypto_secretbox` (XSalsa20-Poly1305) | `XSalsa20Poly1305Cipher` | [Aead/XSalsa20Poly1305Cipher.cs:9](Aead/XSalsa20Poly1305Cipher.cs#L9) | Comment; KAT byte-exact libsodium `secretbox.c`/`secretbox.exp` (tag+ciphertext) + Poly1305 RFC 8439 clamp; mesh NaCl nacltai/cjdns |
...
| RFC 7693 (BLAKE2s) | `Blake2s`, `Blake2sKeyedMac` | [Noise/Blake2s.cs:8](Noise/Blake2s.cs#L8), [Noise/Blake2sKeyedMac.cs:7](Noise/Blake2sKeyedMac.cs#L7) | Comment; KAT BLAKE2s-256 + keyed-KAT (blake2s-kat) |
| RFC 7693 (BLAKE2b) | `Blake2b`, `Blake2bKeyedMac` | [Blake2b.cs:7](Blake2b.cs#L7), [Blake2bKeyedMac.cs:7](Blake2bKeyedMac.cs#L7) | Comment; output 1..64B; KAT App.A `BLAKE2b-512("abc")` + keyed-KAT (blake2b-kat.txt); wrapper BC `Blake2bDigest`, không tự cài compression |
| NaCl `crypto_box` (Curve25519 + XSalsa20-Poly1305) | `NaclBox` | [Aead/NaclBox.cs:5](Aead/NaclBox.cs#L5) | Comment; beforenm = `HSalsa20(X25519,0^16)` + secretbox; KAT byte-exact NaCl `box.c` (tag+ciphertext); hoàn tất bộ NaCl (nacltai/cjdns) |
| RFC 4418 (UMAC) | `Umac` | [Umac.cs:7](Umac.cs#L7) | Comment; UHASH (NH little-endian + POLY mod 2^64-59 + inner-product 2^36-5) ⊕ PDF, KDF/PDF AES-CTR; **KAT §9 byte-exact** (UMAC-32/64/96 mọi độ dài + 128-bit dẫn xuất); single-step POLY khớp vector & reference `umac.c` (RFC ramp p128 KHÔNG dùng — sai lệch spec↔vector); nền fastd |
...
- **NaCl secretbox family ([HSalsa20.cs](HSalsa20.cs) + [XSalsa20.cs](XSalsa20.cs) + [Aead/XSalsa20Poly1305Cipher.cs](Aead/XSalsa20Poly1305Cipher.cs)) — đã hiện thực + KAT byte-exact, chưa có consumer (F.E gap-crypto, mở khóa V.25 mesh NaCl):** `HSalsa20` (core DJB, 20-round no-FF, static) + `XSalsa20` (nonce 24B, subkey=HSalsa20 → **tái dùng `Salsa20`/20** — KHÔNG viết lại core) + `XSalsa20Poly1305Cipher` (= NaCl `crypto_secretbox`, **không** AEAD/AAD; Poly1305 dùng BouncyCastle `Org.BouncyCastle.Crypto.Macs.Poly1305` — cùng package các AEAD khác, r-half clamp RFC 8439 trước `Init` vì BC `CheckKey` đòi khóa đã clamp; verify tag hằng-thời-gian `CryptoBytes.FixedTimeEquals`). **KAT chuẩn**: HSalsa20 khớp libsodium `test/default/core2.c`+`core3.c` SECONDKEY `dc908dda…`; secretbox tag+ciphertext khớp `secretbox.c`/`secretbox.exp` byte-exact ([`XSalsa20Poly1305Tests`](../../tests/TqkLibrary.VpnClient.Crypto.Tests/XSalsa20Poly1305Tests.cs), 9 case). **`crypto_box` nay đã gói** ([Aead/NaclBox.cs](Aead/NaclBox.cs)): `NaclBox` ráp `Curve25519DhGroup` (X25519) + `HSalsa20` (beforenm = `HSalsa20(DH,0^16)`) + `XSalsa20Poly1305Cipher` (secretbox) — **tái dùng nguyên primitive, không viết lại core**; `Box`/`Open` + biến thể `*AfterNm` (precompute beforenm). **KAT byte-exact NaCl `box.c`**: Alice-sk+Bob-pk → beforenm = secretbox firstkey `1b275564…`, box = tag `f3ffc770…` ‖ ciphertext `8e993b9f…` ([`NaclBoxTests`](../../tests/TqkLibrary.VpnClient.Crypto.Tests/NaclBoxTests.cs), 9 case). ⇒ **nacltai/cjdns hết vướng crypto** (còn wire giao thức).
- **BLAKE2b ([Blake2b.cs](Blake2b.cs) + [Blake2bKeyedMac.cs](Blake2bKeyedMac.cs)) — đã hiện thực + KAT chuẩn, chưa có consumer (trám gap-crypto Yggdrasil):** `Blake2b` (unkeyed, output 1..64B mặc định 64) + `Blake2bKeyedMac` (keyed, key ≤64B / output 1..64B) — **wrapper quanh BouncyCastle `Blake2bDigest`** (cùng package `BouncyCastle.Cryptography 2.4.0`), **KHÔNG tự cài compression**; mirror `Blake2s`/`Blake2sKeyedMac` (lưu ý `Blake2bDigest(int)` nhận **BITS**, overload keyed nhận **BYTES**). Đặt ở **root** (không phải `Noise/`) vì là hash đa dụng cho Yggdrasil (node-ID/tree), không phải primitive Noise/WireGuard. **KAT chuẩn**: RFC 7693 App.A `BLAKE2b-512("abc")` = `ba80a53f…`, empty 256/512-bit, keyed `blake2b-kat.txt` (key 00..3f 64B) ([`Blake2bTests`](../../tests/TqkLibrary.VpnClient.Crypto.Tests/Blake2bTests.cs), 9 case). ⇒ **Yggdrasil hết vướng BLAKE2b** (còn wire tree-routing/DHT).
- **P-256 ECDH ([NistP256DhGroup.cs](NistP256DhGroup.cs)) — đã hiện thực + KAT chuẩn, chưa có consumer (trám gap-crypto F.E, mở khóa Nebula-P256/FreeLAN):** `NistP256DhGroup` (`IDhGroup`, IANA **group 19**, RFC 5903) — ECDH trên NIST P-256 (secp256r1/prime256v1): private = 32B scalar trong [1, n-1] (rejection-sampling), public value trên wire = **x‖y 64B** (**KHÔNG** byte point-format 0x04, §7), shared secret = **tọa độ x của g^ir 32B** (§9). **Wrapper quanh BouncyCastle** `CustomNamedCurves("P-256")` + `ECPoint.Multiply` (cùng package `BouncyCastle.Cryptography 2.4.0`), dùng **giống nhau cả 2 TFM** (không `#if` — BCL net8.0 lẫn netstandard2.0 đều không expose raw P-256 scalar-mult/ECDH-agreement, như `Noise/`). Điểm peer decode qua `ECCurve.DecodePoint(0x04‖x‖y)` (tự validate on-curve, ném `ArgumentException` nếu ngoài đường cong) + `IsValid()` (cofactor 1). **KAT byte-exact RFC 5903 §8.1**: `DerivePublicValue(i)`=g^i, `DerivePublicValue(r)`=g^r, `DeriveSharedSecret(i,g^r)`=`DeriveSharedSecret(r,g^i)`=g^irx ([`NistP256DhGroupTests`](../../tests/TqkLibrary.VpnClient.Crypto.Tests/NistP256DhGroupTests.cs), 9 case). ⇒ mở khóa **Nebula-P256 networks + FreeLAN FSCP ECDHE** (còn phần giao thức wire).
