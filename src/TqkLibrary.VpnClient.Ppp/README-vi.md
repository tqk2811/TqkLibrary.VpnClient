...
4. **Rẽ nhánh sau LCP.** `OnLcpOpened`: nếu server đòi MS-CHAPv2 và có authenticator → chờ Challenge; nếu không → đánh dấu đã xác thực và gọi `ReadyForNetworkLayer()` (chạy IPCP + IPV6CP nếu bật). Khi ctor `deferNetworkLayer=true` (PPTP) thì network layer **hoãn** tới khi caller `StartNetworkLayer()` (sau khi CCP/MPPE mở) — [PppEngine.cs:99-152](PppEngine.cs#L99-L152).
...
- **Logging tầng protocol sâu (Q.2):** [`PppEngine`](PppEngine.cs#L14) nhận ctor cuối `ILogger? logger = null` (mặc định `NullLogger` ⇒ **no-op, không đổi hành vi**), tạo các negotiator với logger đó. [`PppNegotiator`](PppNegotiator.cs#L20) base nhận `layer` (`ppp.lcp`/`ppp.ipcp`/`ppp.ipv6cp`) + log Configure-Request/Nak/Reject/Opened; [`LcpNegotiator`](LcpNegotiator.cs#L13) log Echo-Reply + MS-CHAPv2-require; engine log auth-success/-fail + IPCP/IPV6CP-opened. Tất cả ở `LogLevel.Trace` qua seam `VpnLogExtensions.LogProtocolStep` (`Abstractions/Diagnostics`). Driver `Drivers.Sstp`/`Drivers.L2tpIpsec`/`Drivers.Pptp` truyền `Logger` (base supervisor) xuống.
