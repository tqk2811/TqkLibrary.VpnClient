...
> IPsec: ESP transform (RFC 4303) dưới `Esp/`, IKE dưới `Ike/` — IKEv1 Main+Quick Mode (`Ike/V1`) và IKEv2 (`Ike/V2`) — NAT-T UDP framing/channel (RFC 3948, cổng 500/4500) dưới `Nat/`, codec nén payload IP (IPComp, RFC 3173 DEFLATE — **đã wire end-to-end vào IKEv2**) dưới `IpComp/`, và Authentication Header (AH, RFC 4302) dưới `Ah/`.
...
- **IPComp (IP Payload Compression)** — `IpComp/` (RFC 3173, transform DEFLATE RFC 2394/1951): codec **thuần không trạng thái** nén payload IP trước khi giao cho ESP ("compress-then-encrypt", mỗi gói độc lập không giữ history — RFC 2394) qua `System.IO.Compression.DeflateStream` (raw DEFLATE, KHÔNG zlib/gzip wrapper). `TryCompress` áp **non-expansion policy** (RFC 3173 §2.2): chỉ phát IPComp datagram khi nhỏ hơn hẳn payload gốc, ngược lại gửi nguyên gốc (overload nhận CPI tường minh cho outbound, giữ mặc định DEFLATE=2). **Đã wire end-to-end vào IKEv2 (opt-in, mặc định tắt)**: thương lượng qua **IPCOMP_SUPPORTED notify** (16387, RFC 7296 §3.10.1) trong IKE_AUTH — `IpCompSupportedNotify` (CPI ‖ DEFLATE) + `IkeClient.requestIpComp`/`NegotiatedIpCompCpi`; data plane `EspTunnelChannel` compress-then-ESP (Next Header 108) chiều ra + INFLATE chiều vào; responder không chào ⇒ graceful downgrade ESP thường. IKEv1 IPComp chưa (xem §Trạng thái).
- **AH (Authentication Header)** — `Ah/` (RFC 4302): SA cấp **toàn-vẹn + xác thực nguồn gốc** cho gói IP, **KHÔNG mã hóa** (proto-51). `AhSession.Protect` chèn AH sau IP header (transport) hoặc bọc gói trong outer IP header + AH (tunnel) rồi tính ICV trên gói với **trường mutable = 0** (RFC §3.3.3); `TryVerify` tính lại ICV trên bản-sao-zero-mutable, so hằng-thời-gian, áp anti-replay rồi khôi phục gói gốc. ICV dùng thẳng `IIntegrityAlgo` (HMAC-SHA-256-128 / HMAC-SHA1-96). **SA offline as-built — CHƯA wire** vào thương lượng IKE / data plane / builder (xem §Trạng thái).
...
│  ├─ EspTunnelChannel.cs       ESP tunnel mode (RFC 4303) → IPacketChannel: bọc nguyên gói IP (NextHeader 4/41), decap demux theo NextHeader; **IPComp (RFC 3173)** khi có `outboundIpCompCpi`: compress-then-ESP (NH 108) chiều ra + INFLATE chiều vào
...
├─ IpComp/                      IPComp — IP Payload Compression (RFC 3173, DEFLATE) — codec + **wire end-to-end vào IKEv2** (IKEv1 chưa)
│  ├─ IpCompCodec.cs            Codec DEFLATE thuần không trạng thái: TryCompress (non-expansion §2.2, overload CPI tường minh) + Decompress (parse header → INFLATE, chỉ DEFLATE)
│  ├─ Models/IpCompHeader.cs    Header 4B: NextHeader ‖ Flags=0 ‖ CPI big-endian — encode/parse
│  └─ Enums/IpCompTransform.cs  CPI well-known (DEFLATE=2 RFC 2394; OUI/LZS/LZJH liệt kê, chưa hỗ trợ)
│  (notify IKEv2 IPCOMP_SUPPORTED = Ike/V2/Models/IpCompSupportedNotify.cs; data plane = Esp/EspTunnelChannel.cs)
├─ Ah/                          AH — IP Authentication Header (RFC 4302) — SA toàn-vẹn/xác thực (KHÔNG mã hóa), proto-51 — offline (chưa wire)
│  ├─ AhSession.cs              Protect (chèn AH transport / bọc tunnel + ICV zero-mutable) + TryVerify (recompute ICV + FixedTimeEquals + anti-replay + khôi phục gói gốc)
│  ├─ Models/AhHeader.cs        Header cố định 12B: NextHeader ‖ Payload Len §2.2 ‖ Reserved=0 ‖ SPI ‖ Seq — encode/parse; độ dài ICV theo IIntegrityAlgo
│  └─ Helpers/AhMutableFields.cs  Zero trường mutable khi tính ICV: IPv4 §3.3.3.1.1.1 (TOS/Flags/TTL/Checksum) + IPv6 §3.3.3.1.2 (TrafficClass/FlowLabel/HopLimit)
└─ Ike/                         Internet Key Exchange — control plane
   ...
   └─ V2/                       IKEv2 (RFC 7296) — protocol đủ cho V.1 (IKE_SA_INIT/AUTH PSK/EAP/**cert** + CP + multi-TS + INFORMATIONAL + CREATE_CHILD_SA rekey CHILD_SA **và IKE SA**), wire ở Drivers.Ikev2
      ...
      ├─ IkeKeyMaterial.cs      7 khóa SK_d/ai/ar/ei/er/pi/pr (RFC 7296 §2.14) + `DeriveRekey` cho IKE SA rekey (§2.18) + `WithPpk` trộn PPK vào SK_d/pi/pr (RFC 8784 §3)
      ...
      ├─ Models/PpkConfiguration.cs   Cấu hình PPK post-quantum (PPK_ID + PPK + Mandatory + `ToWirePpkId` type-octet §4.1) (RFC 8784)
      ├─ Models/IkeCertificateTrust.cs   Trust anchor verify responder cert: pin leaf hoặc chain tới CA (offline, không dùng OS store)
      ├─ Models/IpCompSupportedNotify.cs   Body notify IPCOMP_SUPPORTED (RFC 7296 §3.10.1): CPI ‖ DEFLATE + `FindDeflateCpi` (thương lượng IPComp cho ESP data plane)
...
### IPComp (data-plane compression, IpComp/)

| Type | Vai trò | Vị trí |
|------|---------|--------|
| `IpCompCodec` (static) | Codec IPComp/DEFLATE thuần không trạng thái: `TryCompress` (DEFLATE + non-expansion §2.2 → `[header ‖ raw-deflate]`, hoặc false ⇒ gửi nguyên gốc; **overload nhận CPI tường minh** cho outbound, mặc định DEFLATE=2) + `Decompress` (parse header → INFLATE, bound `MaxDecompressedSize` chống zip-bomb; CPI≠DEFLATE ⇒ `NotSupportedException`, cụt/hỏng ⇒ `FormatException`) | [IpCompCodec.cs:21](IpComp/IpCompCodec.cs#L21), [TryCompress @ :36](IpComp/IpCompCodec.cs#L36), [overload CPI @ :49](IpComp/IpCompCodec.cs#L49), [Decompress @ :73](IpComp/IpCompCodec.cs#L73) |
| `IpCompHeader` (readonly struct) | Header IPComp 4B (RFC 3173 §3): `NextHeader` ‖ Flags=0 ‖ `Cpi` big-endian; `WriteTo`/`Parse` + ctor CPI tường minh + factory `Deflate(nextHeader)` (CPI=2) | [IpCompHeader.cs:12](IpComp/Models/IpCompHeader.cs#L12), [WriteTo @ :36](IpComp/Models/IpCompHeader.cs#L36), [Parse @ :47](IpComp/Models/IpCompHeader.cs#L47) |
| `IpCompTransform` (enum) | CPI well-known transform id: `Deflate=2` (RFC 2394, đã hiện thực) + `Oui`/`Lzs`/`LzjhV44` (liệt kê, chưa hỗ trợ) | [IpCompTransform.cs:9](IpComp/Enums/IpCompTransform.cs#L9) |
| `IpCompSupportedNotify` (readonly struct) | Body notify **IPCOMP_SUPPORTED** IKEv2 (RFC 7296 §3.10.1): `Cpi` (2B) ‖ `Transform` (1B DEFLATE); `ToNotifyData`/`ToNotifyPayload`/`TryParse` + `FindDeflateCpi` (chọn CPI outbound từ notify responder, thiếu ⇒ null = graceful downgrade) | [IpCompSupportedNotify.cs](Ike/V2/Models/IpCompSupportedNotify.cs) |

> Codec **thuần không trạng thái** (reset mỗi gói — RFC 2394 nén độc lập từng gói, không giữ history), dùng `System.IO.Compression.DeflateStream` cho **raw DEFLATE** (RFC 1951, KHÔNG zlib/gzip wrapper) nên chạy chung cả `netstandard2.0` lẫn `net8.0`. **Đã wire end-to-end vào IKEv2** (opt-in `IkeClient.requestIpComp`, thương lượng IPCOMP_SUPPORTED + data plane `EspTunnelChannel`); IKEv1 IPComp chưa (xem §Trạng thái).

### AH (data-plane integrity, Ah/)

| Type | Vai trò | Vị trí |
|------|---------|--------|
| `AhSession` (sealed) | SA Authentication Header: `Protect(ipPacket, tunnelMode)` chèn AH (transport, IP Protocol→51) hoặc bọc outer IP + AH (tunnel) + tính ICV trên gói zero-mutable; `TryVerify` recompute ICV trên bản-sao-zero-mutable + `CryptoBytes.FixedTimeEquals` + `AntiReplayWindow` rồi khôi phục gói gốc; ctor nhận SPI + key + `IIntegrityAlgo` + tunnel/transport + outer addr | [AhSession.cs:23](Ah/AhSession.cs#L23), [Protect @ :79](Ah/AhSession.cs#L79), [TryVerify @ :94](Ah/AhSession.cs#L94) |
| `AhHeader` (readonly struct) | Header AH cố định 12B (RFC 4302 §2): `NextHeader` ‖ `PayloadLength` (§2.2 = AH/4−2) ‖ Reserved=0 ‖ `SPI` ‖ `Seq`; `WriteFixed`/`Parse` + factory `Create`/`PayloadLengthFor`; `IcvLength` suy từ Payload Len | [AhHeader.cs:12](Ah/Models/AhHeader.cs#L12), [WriteFixed @ :58](Ah/Models/AhHeader.cs#L58), [Parse @ :72](Ah/Models/AhHeader.cs#L72) |
| `AhMutableFields` (static) | Zero trường IP mutable trước khi tính ICV (RFC 4302 §3.3.3.1) trên **bản sao**: `ZeroMutableIpv4` (TOS/Flags+FragOffset/TTL/Checksum §3.3.3.1.1.1) + `ZeroMutableIpv6` (Traffic Class/Flow Label/Hop Limit §3.3.3.1.2) | [AhMutableFields.cs:12](Ah/Helpers/AhMutableFields.cs#L12), [ZeroMutableIpv4 @ :21](Ah/Helpers/AhMutableFields.cs#L21), [ZeroMutableIpv6 @ :38](Ah/Helpers/AhMutableFields.cs#L38) |

> ICV dùng thẳng `IIntegrityAlgo` inject (HMAC-SHA-256-128 = 16B / HMAC-SHA1-96 = 12B — [`HmacIntegrity`](../TqkLibrary.VpnClient.Crypto/HmacIntegrity.cs)); anti-replay tái dùng [`AntiReplayWindow`](../TqkLibrary.VpnClient.Crypto/AntiReplayWindow.cs) của Crypto. **AH KHÔNG mã hóa** (chỉ toàn-vẹn + xác thực nguồn), proto-51 raw **không qua NAT**. **Chưa wire** vào IKE / data plane / builder (xem §Trạng thái). **Caveat**: IPv4 chỉ IHL=5 (không option §3.3.3.1.1.2); IPv6 chỉ base header (không extension §3.3.3.1.2.1) và cần AH bội số 8B (§2.6) ⇒ dùng HMAC-SHA1-96 (16B ICV → 28B AH không align ⇒ `NotSupportedException`).
...
| `IkeKeyMaterial` | 7 khóa SK_d/ai/ar/ei/er/pi/pr: `Derive`=`prf(Ni\|Nr, g^ir)` (§2.14), `DeriveRekey`=`prf(SK_d_old, g^ir\|Ni\|Nr)` cho IKE SA rekey (§2.18); cùng prf+ expand. `WithPpk(ppk)` trả instance MỚI trộn PPK (RFC 8784 §3): `SK_d/SK_pi/SK_pr = prf(PPK, SK_*)`, giữ nguyên SKEYSEED/SK_ai/SK_ar/SK_ei/SK_er | [IkeKeyMaterial.cs:11](Ike/V2/IkeKeyMaterial.cs#L11), [WithPpk @ :107](Ike/V2/IkeKeyMaterial.cs#L107) |
| `PpkConfiguration` | Cấu hình PPK 1 peer (RFC 8784): `PpkId` (giá trị trần) + `Ppk` (khóa) + `Mandatory` + `PpkIdType` (mặc định 1 = PPK_ID_OPAQUE) + `ToWirePpkId()` = type-octet ‖ PpkId (§4.1) | [PpkConfiguration.cs:9](Ike/V2/Models/PpkConfiguration.cs#L9) |
...
| **RFC 3173** (IPComp) + **RFC 2394** (DEFLATE cho IPComp) + **RFC 1951** (raw DEFLATE) + **RFC 7296 §3.10.1** (IPCOMP_SUPPORTED notify IKEv2) | `IpCompCodec` (transform DEFLATE + non-expansion §2.2), `IpCompHeader` (header §3), `IpCompTransform` (CPI = transform id §4.1), `IpCompSupportedNotify` (notify §3.10.1), `EspTunnelChannel` (compress-then-ESP NH 108) | [IpCompCodec.cs:21](IpComp/IpCompCodec.cs#L21), [IpCompHeader.cs:12](IpComp/Models/IpCompHeader.cs#L12), [IpCompTransform.cs:9](IpComp/Enums/IpCompTransform.cs#L9), [IpCompSupportedNotify.cs](Ike/V2/Models/IpCompSupportedNotify.cs), [EspTunnelChannel.cs:20](Esp/EspTunnelChannel.cs#L20) | Có trong comment |
| **RFC 4302** (AH — IP Authentication Header) | `AhHeader` (header §2 + Payload Len §2.2), `AhSession` (ICV §3.3.3, transport+tunnel §3.1.1/§3.1.2, anti-replay §3.4.3), `AhMutableFields` (mutable-field zeroing §3.3.3.1.1.1 IPv4 / §3.3.3.1.2 IPv6) | [AhHeader.cs:12](Ah/Models/AhHeader.cs#L12), [AhSession.cs:23](Ah/AhSession.cs#L23), [AhMutableFields.cs:12](Ah/Helpers/AhMutableFields.cs#L12) | Có trong comment |
...
| **RFC 8784** (PPK post-quantum cho IKEv2) | `IkeKeyMaterial.WithPpk` (trộn khóa §3), `PpkConfiguration` (§4.1 PPK_ID format), `IkeNotifyMessageType.UsePpk/PpkIdentity/NoPpkAuth` (§4), `IkeClient` thương lượng USE_PPK + `ApplyPpkDecision`/`AddPpkAuthNotifies` (§4) | [IkeKeyMaterial.cs:107](Ike/V2/IkeKeyMaterial.cs#L107), [PpkConfiguration.cs:9](Ike/V2/Models/PpkConfiguration.cs#L9), [IkeNotifyMessageType.cs:34](Ike/V2/Enums/IkeNotifyMessageType.cs#L34), [IkeClient.cs:153](Ike/V2/IkeClient.cs#L153) | Có trong comment (initiator; chỉ đường PSK/cert) |
...
- **PPK post-quantum (RFC 8784) — protocol có, driver CHƯA wire:** `IkeClient` nhận `PpkConfiguration? ppk` (opt-in, mặc định null ⇒ không đổi hành vi) để trộn PPK vào SK_d/SK_pi/SK_pr (phía initiator, đường PSK/cert). Đã có test offline self-consistent (round-trip 3 notify, `WithPpk` xác định, ma trận quyết định USE_PPK, full round-trip mock-responder cùng trộn PPK verify AUTH PASS + ca optional-fallback NO_PPK_AUTH). **Còn lại (follow-up)**: (1) thread `PpkConfiguration` từ `Ikev2Driver`/`UseIkev2` xuống `IkeClient` để driver dùng thật; (2) PPK trên đường **EAP** (hiện chỉ PSK/cert); (3) validate live vs strongSwan `ppk`.
...
- **IPComp (RFC 3173, DEFLATE) — codec + WIRE END-TO-END VÀO IKEv2 (opt-in, mặc định tắt):** [`IpComp/`](IpComp) hiện thực codec nén payload IP ([`IpCompCodec`](IpComp/IpCompCodec.cs) — overload CPI tường minh + Decompress DEFLATE-only, [`IpCompHeader`](IpComp/Models/IpCompHeader.cs), [`IpCompTransform`](IpComp/Enums/IpCompTransform.cs)) — DEFLATE raw (RFC 2394/1951) qua `DeflateStream`, non-expansion policy §2.2, compress-then-encrypt trước ESP. **Wire IKEv2**: thương lượng qua **IPCOMP_SUPPORTED notify (16387, RFC 7296 §3.10.1)** — [`IpCompSupportedNotify`](Ike/V2/Models/IpCompSupportedNotify.cs) (CPI ‖ DEFLATE + `FindDeflateCpi`); [`IkeClient`](Ike/V2/IkeClient.cs) `requestIpComp` gắn notify cạnh CHILD_SA trong IKE_AUTH (PSK **và** EAP) + parse notify responder ⇒ `NegotiatedIpCompCpi`. Ta quảng bá **CPI inbound = 2 (DEFLATE well-known)** để `Decompress` (chỉ DEFLATE) nhận gói vào; stamp **CPI peer** cho gói ra. Data plane [`EspTunnelChannel`](Esp/EspTunnelChannel.cs) nhận `outboundIpCompCpi`: outbound compress-then-ESP (Next Header 108 = [`EspConstants.NextHeaderIpComp`](Esp/EspConstants.cs)), rơi về ESP thường nếu non-expansion; inbound NH=108 ⇒ INFLATE. Responder không chào ⇒ **graceful downgrade** ESP thường (0 thay đổi hành vi PSK/EAP đã live). Test offline: [`IpCompCodecTests`](../../tests/TqkLibrary.VpnClient.Ipsec.IpComp.Tests/IpCompCodecTests.cs) (23 case) + [`IkeV2IpCompNotifyTests`](../../tests/TqkLibrary.VpnClient.Ipsec.Ike.Tests/IkeV2IpCompNotifyTests.cs) (notify encode/parse + graceful) + [`EspTunnelChannelTests`](../../tests/TqkLibrary.VpnClient.Ipsec.Esp.Tests/EspTunnelChannelTests.cs) (data-plane round-trip + non-expansion downgrade + inactive passthrough). **Còn lại**: IKEv1 IPComp (Phase 2 proposal) + validate live vs strongSwan `compress=yes`.
- **AH (RFC 4302) — SA offline, CHƯA wire:** [`Ah/`](Ah) hiện thực [`AhSession`](Ah/AhSession.cs) (`Protect`/`TryVerify` transport + tunnel, IPv4 + IPv6), [`AhHeader`](Ah/Models/AhHeader.cs) và [`AhMutableFields`](Ah/Helpers/AhMutableFields.cs) — toàn-vẹn + xác thực nguồn gốc **không mã hóa** (proto-51). ICV tính trên gói với **trường mutable = 0** (§3.3.3), verify recompute + `CryptoBytes.FixedTimeEquals` + [`AntiReplayWindow`](../TqkLibrary.VpnClient.Crypto/AntiReplayWindow.cs) (cửa sổ 64). **Đã as-built + test offline tự-hiển-nhiên-đúng** (24 case: round-trip transport+tunnel IPv4/IPv6, **đổi TTL/checksum/DSCP trên dây ⇒ vẫn verify** vs **đổi source/payload immutable ⇒ fail** — bằng chứng zero đúng field, cả HMAC-SHA-256-128 lẫn SHA1-96, anti-replay trùng/quá-cũ/tiến — [`AhSessionTests`](../../tests/TqkLibrary.VpnClient.Ipsec.Ah.Tests/AhSessionTests.cs)). **Caveat**: IPv4 chỉ IHL=5 (chưa xử option §3.3.3.1.1.2); IPv6 chỉ base header (chưa xử extension header §3.3.3.1.2.1) và AH phải bội số 8B (§2.6) ⇒ HMAC-SHA-256-128 (28B AH) ném `NotSupportedException` cho IPv6, dùng HMAC-SHA1-96 (24B). **Còn lại**: thương lượng AH SA trong IKE (AH proposal) + cắm vào data plane + builder — AH ít dùng thực tế (proto-51 raw không qua NAT, ESP thắng thế) nên ưu tiên thấp.
