...
> IPsec: ESP transform (RFC 4303) dưới `Esp/`, IKE dưới `Ike/` — IKEv1 Main+Quick Mode (`Ike/V1`) và IKEv2 (`Ike/V2`) — NAT-T UDP framing/channel (RFC 3948, cổng 500/4500) dưới `Nat/`, codec nén payload IP (IPComp, RFC 3173 DEFLATE) dưới `IpComp/`, và Authentication Header (AH, RFC 4302) dưới `Ah/`.
...
- **IPComp (IP Payload Compression)** — `IpComp/` (RFC 3173, transform DEFLATE RFC 2394/1951): codec **thuần không trạng thái** nén payload IP trước khi giao cho ESP ("compress-then-encrypt", mỗi gói độc lập không giữ history — RFC 2394) qua `System.IO.Compression.DeflateStream` (raw DEFLATE, KHÔNG zlib/gzip wrapper). `TryCompress` áp **non-expansion policy** (RFC 3173 §2.2): chỉ phát IPComp datagram khi nhỏ hơn hẳn payload gốc, ngược lại gửi nguyên gốc. **Codec offline as-built — CHƯA wire** vào thương lượng IKE / ESP data plane / builder (xem §Trạng thái).
...
├─ IpComp/                      IPComp — IP Payload Compression (RFC 3173, DEFLATE) — codec data-plane offline (chưa wire vào IKE/ESP)
│  ├─ IpCompCodec.cs            Codec DEFLATE thuần không trạng thái: TryCompress (non-expansion §2.2) + Decompress (parse header → INFLATE)
...
| Type | Vai trò | Vị trí |
|------|---------|--------|
| `IpCompCodec` (static) | Codec IPComp/DEFLATE thuần không trạng thái: `TryCompress` (DEFLATE + non-expansion §2.2 → `[header ‖ raw-deflate]`, hoặc false ⇒ gửi nguyên gốc) + `Decompress` (parse header → INFLATE, bound `MaxDecompressedSize` chống zip-bomb; CPI≠DEFLATE ⇒ `NotSupportedException`, cụt/hỏng ⇒ `FormatException`) | [IpCompCodec.cs:21](IpComp/IpCompCodec.cs#L21), [TryCompress @ :36](IpComp/IpCompCodec.cs#L36), [Decompress @ :60](IpComp/IpCompCodec.cs#L60) |
| `IpCompHeader` (readonly struct) | Header IPComp 4B (RFC 3173 §3): `NextHeader` ‖ Flags=0 ‖ `Cpi` big-endian; `WriteTo`/`Parse` + factory `Deflate(nextHeader)` (CPI=2) | [IpCompHeader.cs:12](IpComp/Models/IpCompHeader.cs#L12), [WriteTo @ :36](IpComp/Models/IpCompHeader.cs#L36), [Parse @ :47](IpComp/Models/IpCompHeader.cs#L47) |
...
> Codec **thuần không trạng thái** (reset mỗi gói — RFC 2394 nén độc lập từng gói, không giữ history), dùng `System.IO.Compression.DeflateStream` cho **raw DEFLATE** (RFC 1951, KHÔNG zlib/gzip wrapper) nên chạy chung cả `netstandard2.0` lẫn `net8.0`. **Chưa wire** vào thương lượng IKE / ESP data plane / builder — mới là codec offline (xem §Trạng thái).
...
| **RFC 3173** (IPComp) + **RFC 2394** (DEFLATE cho IPComp) + **RFC 1951** (raw DEFLATE) | `IpCompCodec` (transform DEFLATE + non-expansion §2.2), `IpCompHeader` (header §3), `IpCompTransform` (CPI = transform id §4.1) | [IpCompCodec.cs:21](IpComp/IpCompCodec.cs#L21), [IpCompHeader.cs:12](IpComp/Models/IpCompHeader.cs#L12), [IpCompTransform.cs:9](IpComp/Enums/IpCompTransform.cs#L9) | Có trong comment |
...
- **IPComp (RFC 3173, DEFLATE) — codec offline, CHƯA wire:** [`IpComp/`](IpComp) hiện thực codec nén payload IP ([`IpCompCodec`](IpComp/IpCompCodec.cs), [`IpCompHeader`](IpComp/Models/IpCompHeader.cs), [`IpCompTransform`](IpComp/Enums/IpCompTransform.cs)) — DEFLATE raw (RFC 2394/1951) qua `DeflateStream`, non-expansion policy §2.2, compress-then-encrypt trước ESP. **Đã as-built + test offline tự-hiển-nhiên-đúng** (round-trip nhiều cỡ/kiểu payload + non-expansion dữ liệu ngẫu nhiên + parse header/CPI/truncation/corrupt-stream — [`IpCompCodecTests`](../../tests/TqkLibrary.VpnClient.Ipsec.IpComp.Tests/IpCompCodecTests.cs)). **Còn lại**: thương lượng IPComp SA trong IKE (IKEv1 IPCOMP_SUPPORTED / IKEv2 Notify) + cắm vào ESP data plane (compress trước `EspSession.Protect`, decompress sau `TryUnprotect` theo NextHeader IPComp) — chưa có nên chưa đổi đường live.
