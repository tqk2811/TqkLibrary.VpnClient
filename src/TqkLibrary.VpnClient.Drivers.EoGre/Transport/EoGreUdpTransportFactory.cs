...
namespace TqkLibrary.VpnClient.Drivers.EoGre.Transport
{
    /// <summary>
    /// The production <see cref="IEoGreTransportFactory"/>: opens a real connected UDP socket to the remote endpoint.
    /// GRE-in-UDP is UDP-only — one datagram is one message, no framing — so the transport is a thin
    /// <see cref="IDatagramTransport"/> plus a receive loop that raises each datagram to the connection's handler. It binds
    /// an ephemeral local port and connects to the remote (IPv4 or IPv6, following the endpoint's
    /// <see cref="AddressFamily"/>). The socket I/O is exercised live (lab); the offline tests drive the connection through
    /// an in-process factory. Mirrors <c>GeneveUdpTransportFactory</c>.
    /// </summary>
    public sealed class EoGreUdpTransportFactory : IEoGreTransportFactory
    {
        ...
        /// </summary>
        public EoGreUdpTransportFactory(int receiveBufferSize = 65535, IPAddress? localBind = null)
        {
        ...
        /// <inheritdoc/>
        public async Task<EoGreTransportHandle> ConnectAsync(IPEndPoint remote, CancellationToken cancellationToken)
        {
            ...
            return new EoGreTransportHandle(socket, socket.SetReceiver, socket.RunReceiveLoopAsync);
        }

        /// <summary>A connected UDP datagram pipe over a real socket (live-only). Binds an ephemeral local port and
        /// connects to the remote endpoint; supports IPv4 and IPv6.</summary>
