...
| `OpenVpnKeyNegotiation` | client: `NegotiateAsync(options, user?, pass?, peerInfo?)` ghi msg + đọc reply trên `Stream` → `OpenVpnKeyMaterial`; lộ `ServerOptions` (OCC options server trả, có thể chứa IV_CIPHERS) | [DataChannel/OpenVpnKeyNegotiation.cs:11](DataChannel/OpenVpnKeyNegotiation.cs#L11) |
| `OpenVpnKeyMaterial` | key2 256B cipher-independent; `DeriveDataKeys(cipher, isServer?)` (AEAD) / `DeriveCbcDataKeys(cbcCipher, hmacKeyLen, isServer?)` (CBC) slice sau khi biết cipher (NCP) | [DataChannel/OpenVpnKeyMaterial.cs:9](DataChannel/OpenVpnKeyMaterial.cs#L9) |
