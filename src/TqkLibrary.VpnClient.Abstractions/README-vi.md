...
| `TunnelConfig` | Kết quả mạng của session: địa chỉ IPv4 + prefix, **địa chỉ IPv6 + prefix (`AssignedAddressV6`/`PrefixLengthV6`, P1.1 — null nếu không bật IPv6)**, **default gateway `Gateway`/`GatewayV6`** (dạng có kiểu của route `0.0.0.0/0`/`::/0` trong `Routes` — cầu L2 cần next-hop mà không phải parse chuỗi route), DNS, routes, MTU (chỉ dùng nội bộ, không ghi routing table OS) | [TunnelConfig.cs:9](Drivers/Models/TunnelConfig.cs#L9) |
