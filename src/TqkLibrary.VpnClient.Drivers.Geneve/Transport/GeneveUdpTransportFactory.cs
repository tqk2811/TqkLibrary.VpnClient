...
{
    /// <summary>
    /// The production <see cref="IGeneveTransportFactory"/>: opens a real connected UDP socket to the remote endpoint.
    /// Geneve is UDP-only — one datagram is one message, no framing — so the transport is a thin
    ...
    /// </summary>
    public sealed class GeneveUdpTransportFactory : IGeneveTransportFactory
    {
        ...
        /// </summary>
        public GeneveUdpTransportFactory(int receiveBufferSize = 65535, IPAddress? localBind = null)
        {
        ...
        /// <inheritdoc/>
        public async Task<GeneveTransportHandle> ConnectAsync(IPEndPoint remote, CancellationToken cancellationToken)
        {
            ...
            return new GeneveTransportHandle(socket, socket.SetReceiver, socket.RunReceiveLoopAsync);
        }
