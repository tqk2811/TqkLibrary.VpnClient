...
namespace TqkLibrary.VpnClient.Drivers.Geneve.DataChannel
{
    /// <summary>
    /// The Geneve data plane as an L2 <see cref="IEthernetChannel"/>: it carries full Ethernet frames behind an 8-byte
    /// Geneve base header (RFC 8926, protocol type 0x6558 Transparent Ethernet Bridging) over the UDP transport, so it
    /// plugs straight into the userspace Ethernet fabric (ARP + the <c>VirtualHost</c> bridge), which then bridges down to
    /// the IP stack — the stack never binds here directly. Because the payload is a complete Ethernet frame,
    ...
    /// <para>
    /// Egress (<see cref="WriteFrameAsync"/>) prepends the Geneve base header (with the configured VNI, no options) and
    /// hands the datagram to the supplied <c>sink</c> (the connection's transport write, to the static remote endpoint).
    /// Ingress is push-driven: the connection's receive loop decodes inbound Geneve datagrams and calls
    /// <see cref="Deliver"/> with the recovered Ethernet frame, which raises <see cref="InboundFrame"/>. This type holds no
    /// socket itself, mirroring <c>VxlanEthernetChannel</c> — but the emitted header always advertises protocol type
    /// 0x6558 and OptLen 0 (a client in L2 mode adds no options).
    /// </para>
    /// </summary>
    internal sealed class GeneveEthernetChannel : IEthernetChannel
    {
        readonly uint _vni;
        ...
        /// <summary>
        /// Wires the channel. <paramref name="vni"/> stamps every outbound Geneve header; <paramref name="localMac"/> is
        ...
        /// </summary>
        public GeneveEthernetChannel(uint vni, MacAddress localMac,
        ...
        public ValueTask WriteFrameAsync(ReadOnlyMemory<byte> ethernetFrame, CancellationToken cancellationToken = default)
        {
            ...
            byte[] datagram = GeneveCodec.EncodeGeneve(_vni, GeneveCodec.ProtocolTypeTransparentEthernet, ethernetFrame.Span);
            return _sink(datagram, cancellationToken);
        }

        /// <summary>
        /// Surfaces one inbound Ethernet frame to the fabric. The connection's receive loop calls this for each Geneve
