...
- **Hạn chế đã biết / việc sau:**
  ...
  - **Validate live (2026-06-23, P0.8c, lab `l2tp-nonat/` strongSwan no-NAT, Docker bridge):** raw proto-50 socket mở OK + gửi ESP native (tcpdump proto50>0, udp4500=0) — **phát hiện + sửa bug `NativeRawSocket` EPROTONOSUPPORT** (managed `Socket(Raw,proto-50)` fail trên Linux dù kernel hỗ trợ; TRƯỚC fix native-ESP/GRE 100% hỏng trên Linux). End-to-end của driver L2TP còn hoãn vì **server** strongSwan cài SA `espinudp` (xem [README Drivers.L2tpIpsec](../TqkLibrary.VpnClient.Drivers.L2tpIpsec/README-vi.md)) — không phải lỗi transport này. Windows best-effort vẫn chờ kiểm; IPv6 receive chờ **Q.1**.
