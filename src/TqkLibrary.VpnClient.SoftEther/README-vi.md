...
├─ SoftEtherHttpPackCodec.cs   — V4.b framing PACK trong thân HTTP (PACK bytes verbatim, Content-Length phân định; build POST/200 + parse body)
...
├─ PrefixedByteStreamTransport.cs — V4.c decorator IByteStreamTransport: phục vụ prefix (byte handshake reader over-read qua ranh body) trước rồi fall-through inner — seam handshake↔data session, write/connect/dispose pass-through
...
| `SoftEtherHttpPackCodec` | framing PACK trong thân HTTP: `FrameBody`/`ParseBody` (PACK bytes verbatim, Content-Length phân định) + `BuildPostRequest`/`BuildOkResponse` | [SoftEtherHttpPackCodec.cs:15](SoftEtherHttpPackCodec.cs#L15) |
...
| `PrefixedByteStreamTransport` | V4.c decorator `IByteStreamTransport`: `ReadAsync` phục vụ prefix (byte handshake reader over-read qua ranh HTTP body vào data session) drain trái-sang-phải rồi fall-through `Inner`; write/connect/dispose pass-through (không thread-safe đọc đồng thời) | [PrefixedByteStreamTransport.cs:18](PrefixedByteStreamTransport.cs#L18) |
...
2. **Đọc hello** ([`ReadHttpPackAsync`](SoftEtherHandshake.cs#L242) → [`ParseHello`](SoftEtherHandshake.cs#L41)): đọc 1 message HTTP (header tới `\r\n\r\n` + `Content-Length`), parse thân qua [`SoftEtherHttpPackCodec.ParseBody`](SoftEtherHttpPackCodec.cs#L30) → PACK `hello`/`version`/`build`/`random`(20B challenge).
3. **Dựng & gửi login** ([`BuildLoginPack`](SoftEtherHandshake.cs#L69) → [`SoftEtherHttpPackCodec.BuildPostRequest`](SoftEtherHttpPackCodec.cs#L37)): PACK `method=login`/`hubname`/`username`/`authtype` + credential + session params + **danh tính client** (`client_str`/`client_ver`/`client_build`, mặc định ở [`SoftEtherProtocol`](SoftEtherProtocol.cs#L82), ghi đè qua [`SoftEtherLoginRequest`](Models/SoftEtherLoginRequest.cs#L30)). **Auth password**: [`SoftEtherAuth.ComputeSecurePassword`](SoftEtherAuth.cs#L76) = `SHA0(SHA0(password‖UPPER(username))‖server_random)` (tái dùng [`Sha0`](../TqkLibrary.VpnClient.Crypto/Sha0.cs) qua `IHashAlgo`) — password gốc **không** qua wire. POST tới `/vpnsvc/vpn.cgi`.
   - **Ba trường danh tính KHÔNG phải thông tin trang trí.** Thiếu chúng, server vẫn nhận login và trả welcome PACK đầy đủ (session key, policy, max_connection…), rồi **giết phiên dữ liệu ở tầng TLS** ngay block đầu tiên bằng alert `protocol_version` — không có mã lỗi giao thức nào chỉ ra nguyên nhân. Đo trên VPN Gate 10/09/2026: thêm đúng 3 trường này là data session sống, các nhóm trường khác (`hello`/`version`/`build`, `protocol`/`client_id`/`qos`/`support_*`) không thay đổi gì.
