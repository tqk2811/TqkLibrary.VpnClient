...
| --- | --- | --- | --- |
| [MS-SSTP] (Secure Socket Tunneling Protocol) | `SstpDriver` qua `UseSstp()` | [VpnClientBuilder.cs:19-20](VpnClientBuilder.cs#L19-L20), [SstpDriver.cs:7-8](../TqkLibrary.Vpn.Drivers/Sstp/SstpDriver.cs#L7-L8) | Comment "MS-SSTP driver" ở façade; driver mô tả "TLS over 443, PPP, MS-CHAPv2" |
| RFC 2759 (MS-CHAPv2) | `SstpDriver` + `L2tpIpsecDriver` (PPP auth) | [VpnClientBuilder.cs:20](VpnClientBuilder.cs#L20), [VpnClientBuilder.cs:23](VpnClientBuilder.cs#L23) | (suy luận) — hiện thực thực tế ở Ppp/`MsChapV2` |
| RFC 2409 (IKEv1 / ISAKMP) | `L2tpIpsecDriver` qua `UseL2tpIpsec()` | [VpnClientBuilder.cs:22-23](VpnClientBuilder.cs#L22-L23), [L2tpIpsecDriver.cs:8](../TqkLibrary.Vpn.Drivers/L2tpIpsec/L2tpIpsecDriver.cs#L8) | (suy luận) — comment driver "IKEv1 PSK over NAT-T"; logic ở Ipsec `Ike/V1` |
...
| RFC 1661/1332 (PPP/IPCP) | cả 2 driver (`UsesPpp`, `AddressAssignment.Ipcp`) | [SstpDriver.cs:17-22](../TqkLibrary.Vpn.Drivers/Sstp/SstpDriver.cs#L17-L22), [L2tpIpsecDriver.cs:29-34](../TqkLibrary.Vpn.Drivers/L2tpIpsec/L2tpIpsecDriver.cs#L29-L34) | (suy luận) — IP cấp qua IPCP; hiện thực ở Ppp |
...
Toàn bộ việc lắp ráp stack (IKE → ESP → L2TP → PPP → IP) diễn ra **bên trong driver**, không nằm ở project này. Ví dụ điều phối L2TP/IPsec: `L2tpIpsecDriver.ConnectAsync` dựng `L2tpIpsecConnection` rồi gọi `ConnectAsync` của nó — [L2tpIpsecDriver.cs:38-59](../TqkLibrary.Vpn.Drivers/L2tpIpsec/L2tpIpsecDriver.cs#L38-L59). Chi tiết luồng handshake xem [.docs/10 §6](../../.docs/10-codebase-architecture-and-flow.md).
