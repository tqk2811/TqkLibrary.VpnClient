...
- **Data plane (L2)**: [`ZeroTierEthernetChannel`](DataChannel/ZeroTierEthernetChannel.cs) (`IEthernetChannel`): egress đọc dst/src-MAC từ header Ethernet → đóng frame thành **EXT_FRAME** (+ đính COM lần đầu) → seal Salsa20/12 VL1 → gửi UDP; ingress (EXT_FRAME/FRAME đã decode) → `InboundFrame`. Cắm vào [`ZeroTierNeighborResolver`](DataChannel/ZeroTierNeighborResolver.cs) (derive peer MAC, không ARP) + [`VirtualHost`](../TqkLibrary.VpnClient.Ethernet/VirtualHost.cs) (L2↔L3) → `IPacketChannel`.
...
| Dùng | [ZeroTier](../TqkLibrary.VpnClient.ZeroTier) | `Vl1PacketCodec` (seal/open), `HelloMessageCodec`/`OkMessageCodec`/`InetAddressCodec`, `Vl1KeyDerivation`, `NetworkConfigCodec`/`ZeroTierDictionary`, `Vl2FrameCodec`/`Vl2ExtFrameCodec`, models identity/network/frame |
...
├─ DataChannel/
│  ├─ ZeroTierEthernetChannel.cs      IEthernetChannel: WriteFrameAsync đóng Ethernet frame thành EXT_FRAME (+COM lần đầu) + seal VL1; DeliverExtFrame/DeliverFrame raise InboundFrame
...
- **CÒN LẠI — data plane VL2 ICMP live 2 chiều**: MAC node ZeroTier là **random per-node tap MAC** (KHÔNG derivable từ ZT address — `listnetworks` MAC ≠ `DeriveMac`) ⇒ cần **MULTICAST_FRAME (verb 0x0e)** cho broadcast/ARP để học MAC peer, hoặc learn MAC từ frame inbound. EXT_FRAME egress 2 chiều đã proven **offline** (6 test). Hiện `ZeroTierNeighborResolver` derive MAC (đúng model ZeroTier về nguyên tắc, đúng offline) nhưng node thật dùng random tap MAC.
...
- **Refs**: Abstractions + Crypto + Drivers.Core + Ethernet + ZeroTier. **2 TFM** (netstandard2.0 + net8.0). **1 IVpnSession** (`OpenSessionAsync` ⇒ `NotSupported`). Demo: `--vpn <file>.zerotier` ([`ConnectZeroTierAsync`](../../demo/Vpn2ProxyDemo/VpnTunnel.cs)).
