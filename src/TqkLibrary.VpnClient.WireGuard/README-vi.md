...
| `WireGuardConfig` | (V3.f) config tĩnh: `PrivateKey` + `Address`/`AddressV6`/`PrefixLength*` + `DnsServers` + `Mtu`(1420); **single-peer** qua `PeerPublicKey?`/`PresharedKey?`/`AllowedIps` (mặc định `0.0.0.0/0,::/0`)/`PersistentKeepaliveSeconds`, **hoặc multi-peer** qua `Peers` (`IReadOnlyList<WireGuardPeer>`); `EnumeratePeers()` chuẩn hóa (Peers nếu có, ngược lại 1 peer từ trường tiện ích — không có cả hai ⇒ ném); `ToTunnelConfig()` → `TunnelConfig` tĩnh (routes = hợp allowed-ips mọi peer) | [Config/WireGuardConfig.cs:24](Config/WireGuardConfig.cs#L24) |
| `WireGuardPeer` | (V.3) một `[Peer]`: `PublicKey` + `PresharedKey?` + `AllowedIps` (CIDR v4+v6, mặc định `0.0.0.0/0,::/0`) + `PersistentKeepaliveSeconds` + `Endpoint?` (per-peer, null ⇒ dùng endpoint connection) | [Config/WireGuardPeer.cs:14](Config/WireGuardPeer.cs#L14) |
| `IpPrefix` | (V.3) value-object CIDR bất biến (struct): `TryParse` (`net/prefix` hoặc host ⇒ /32 hoặc /128, mask host-bits, lệch family ⇒ false), `Contains(IPAddress)` so `PrefixLength` bit đầu, `PrefixLength` = độ đặc hiệu longest-prefix; `Equals`/`GetHashCode`/`ToString` | [Routing/IpPrefix.cs:20](Routing/IpPrefix.cs#L20) |
...
| `WireGuardCryptoRouter` | (V.3) bảng route longest-prefix bất biến: `Build(per-peer allowed-ips)` (skip CIDR sai), `TryRoute(dest, out peerIndex)` chọn prefix dài nhất phủ dest (tie ⇒ peer thêm trước; không phủ ⇒ false), `Count` | [Routing/WireGuardCryptoRouter.cs:18](Routing/WireGuardCryptoRouter.cs#L18) |
...
1. **Build bảng route**: [`WireGuardCryptoRouter.Build`](Routing/WireGuardCryptoRouter.cs#L36) nhận danh sách allowed-ips theo peer (peer i → CIDR list), parse từng CIDR thành [`IpPrefix`](Routing/IpPrefix.cs#L20) (skip cái sai), giữ thứ tự peer.
2. **Gửi** ([`WireGuardMultiPeerChannel.WriteIpPacketAsync`](Transport/WireGuardMultiPeerChannel.cs#L84)): [`IpPacketDestination.TryGetDestination`](Routing/IpPacketDestination.cs#L23) đọc dest từ goi IP → [`TryRoute`](Routing/WireGuardCryptoRouter.cs#L62) chọn prefix **dài nhất** phủ dest → seal qua `WireGuardChannel` của peer đó. **Tie** độ dài prefix ⇒ peer thêm trước thắng (giống "first matching most-specific" của WireGuard). **Không peer nào phủ** ⇒ drop (báo `onNoRoute`, driver log `VpnDropReason.NoRoute`). **Single-peer** (point-to-point, 1 peer `0.0.0.0/0,::/0`) ⇒ mọi gói về peer 0, không bắt buộc parse IP header.
3. **Nhận** ([`Deliver`](Transport/WireGuardMultiPeerChannel.cs#L106)): gói type-4 mang receiver-index của session đích nên thử lần lượt từng peer channel `Deliver` (kiểm index của mình + tag) đến khi một peer mở được → trả index peer (gói inbound từ peer nào gán về peer đó); không peer nào mở ⇒ −1 (foreign/forged/replayed).
4. **Rekey**: [`SetPeerChannel(i, channel)`](Transport/WireGuardMultiPeerChannel.cs#L70) hot-swap channel của **một** peer (make-before-break per-peer) — các peer khác giữ nguyên, facade trước channel không rebind.
