...
dotnet run --project demo/Vpn2ProxyDemo -- http-request --vpn ikev2://vpn@<IP>?psk=vpn --ipcomp   # IKEv2 + IPComp DEFLATE (RFC 3173): server compress=yes ⇒ nén payload trước ESP (gửi trang lớn để ép nén)
dotnet run --project demo/Vpn2ProxyDemo -- dns --vpn ikev2://vpn@<IP>?psk=vpn --ppk s3cr3t --ppk-id ppk1   # IKEv2 + PPK post-quantum (RFC 8784): trộn PPK vào SK_d/SK_pi/SK_pr (gateway strongSwan cấu hình ppk cùng id/secret); '0x…' = hex
...
| `--ipcomp` | `false` | **(Chỉ IKEv2)** thương lượng **IPComp nén payload** (RFC 3173 DEFLATE) qua **IPCOMP_SUPPORTED** trong IKE_AUTH (RFC 7296 §3.10.1). Gateway phải cùng chào (strongSwan `compress=yes`) — nếu không tunnel chạy **ESP thường** (graceful downgrade). Nén chỉ áp khi gói co lại được (non-expansion §2.2) ⇒ gửi lưu lượng lớn/nén được để ép nén. Scheme khác IKEv2 ⇒ bỏ qua |
| `--ppk` | *(rỗng)* | **(Chỉ IKEv2)** bí mật **Post-quantum Preshared Key** (RFC 8784) trộn vào SK_d/SK_pi/SK_pr — client chào **USE_PPK** (IKE_SA_INIT) rồi **PPK_IDENTITY** (IKE_AUTH). Chuỗi **UTF-8** hoặc **`0x`+hex** ⇒ byte thô. Chỉ đường **PSK** (`?psk=`); **KHÔNG** dùng chung `--ikev2-eap`. Gateway phải cấu hình cùng `ppk` (strongSwan swanctl). Scheme khác IKEv2 ⇒ bỏ qua. Rỗng ⇒ không dùng PPK |
| `--ppk-id` | `ppk1` | **(Chỉ IKEv2, kèm `--ppk`)** định danh PPK (**PPK_ID**, RFC 8784 §4.1) gửi trong PPK_IDENTITY (wire = octet type `1` ‖ id). Chuỗi UTF-8 hoặc `0x`+hex. Phải **khớp** id cấu hình trên gateway |
| `--ppk-mandatory` | `false` | **(Chỉ IKEv2, kèm `--ppk`)** **bắt buộc** PPK: gateway không chào USE_PPK ⇒ **hủy** kết nối. Mặc định tắt ⇒ **tùy chọn** (server từ chối ⇒ fallback PSK thường kèm **NO_PPK_AUTH**) |
