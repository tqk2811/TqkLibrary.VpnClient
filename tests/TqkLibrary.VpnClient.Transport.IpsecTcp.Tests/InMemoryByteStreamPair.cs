...
namespace TqkLibrary.VpnClient.Transport.IpsecTcp.Tests
{
    ...
    /// <see cref="WriteAsync"/> enqueues a copied chunk that the peer's <see cref="ReadAsync"/> drains (returning up to
    /// the caller's buffer size, so the reassembler still sees arbitrary read boundaries). No sockets, no TLS — test
    /// scaffolding only.
    /// </summary>
