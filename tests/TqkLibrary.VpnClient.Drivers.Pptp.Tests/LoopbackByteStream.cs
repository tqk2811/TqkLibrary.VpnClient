...
namespace TqkLibrary.VpnClient.Drivers.Pptp.Tests
{
    ...
    /// land on the peer's read buffer. Stands in for the TCP/1723 control connection so the PPTP driver can be driven
    /// against a simulated PAC offline. Throwaway test scaffolding (the library is the PNS/client; the PAC responder
    /// role exists only here). Replicated from the Pptp.Tests project's helper.
    /// </summary>
