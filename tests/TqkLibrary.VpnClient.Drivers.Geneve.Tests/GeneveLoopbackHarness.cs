...
{
    /// <summary>
    /// An in-memory connected UDP loopback that ties the real <see cref="GeneveConnection"/> to an in-process peer built
    /// from the same protocol blocks (<see cref="GeneveCodec"/> + the Ethernet/ARP codecs). Lossless + ordered, each send
    ...
    /// <summary>An <see cref="IGeneveTransportFactory"/> that hands back a fixed in-process pipe (self-pumping loopback).</summary>
    sealed class InProcessGeneveTransportFactory : IGeneveTransportFactory
    {
        readonly LoopbackUdpLink.Endpoint _endpoint;
        public InProcessGeneveTransportFactory(LoopbackUdpLink.Endpoint endpoint) => _endpoint = endpoint;

        public Task<GeneveTransportHandle> ConnectAsync(IPEndPoint remote, CancellationToken cancellationToken)
            => Task.FromResult(new GeneveTransportHandle(_endpoint, _endpoint.SetReceiver, receivePump: null));
    }

    /// <summary>
    /// A throwaway Geneve peer + gateway: for every inbound Geneve datagram it decodes the encapsulated Ethernet frame,
    /// answers ARP for the gateway, and echoes inbound IPv4 unicast frames back (swapping MAC src/dst) — re-wrapping each
    /// reply as a Geneve datagram (protocol type 0x6558, no options) under the same VNI. Pure RFC 8926 behaviour; no
    /// external source.
    /// </summary>
    sealed class SimulatedGenevePeer : IDisposable
    {
        ...
        public SimulatedGenevePeer(LoopbackUdpLink.Endpoint transport, uint vni)
        {
        ...
        {
            if (!GeneveCodec.TryDecodeGeneve(datagram.Span, out uint vni, out ushort protocolType, out ReadOnlyMemory<byte> frameMem)) return;
            if (vni != _vni || protocolType != GeneveCodec.ProtocolTypeTransparentEthernet) return;
            DatagramCount++;
            ...
            _ = _transport.SendAsync(GeneveCodec.EncodeGeneve(_vni, GeneveCodec.ProtocolTypeTransparentEthernet, reply));
        }
